...
window.__BP_WORLD_RENDER_VERSION__=5356;
...
function style(){
 ...
 layers:[
  ...
  {id:'gta-context-buildings',type:'fill-extrusion',source:'ofm','source-layer':'building',minzoom:11.2,paint:{'fill-extrusion-color':['interpolate',['linear'],['zoom'],11.2,'#767d80',15,'#83898b',18,'#8d9293'],'fill-extrusion-height':['max',4,bHeight],'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','render_min_height']],['to-number',['get','min_height']],0]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],11.2,.82,13,.9,15,.97,18,1],'fill-extrusion-vertical-gradient':true}},
  ...
  {id:'gta-bp-roofs',type:'fill-extrusion',source:'bpBuildings','source-layer':'buildings',minzoom:MOBILE?15.8:13.8,paint:{'fill-extrusion-color':'#666c6d','fill-extrusion-base':['max',['coalesce',['to-number',['get','base_height_m']],0],['-',['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],['max',.18,['coalesce',['to-number',['get','roof_height_m']],.3]]]],'fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],13.8,.8,15,.94,18,.995],'fill-extrusion-vertical-gradient':true}},
  ...
  {id:'gta-exact-building',type:'fill-extrusion',source:'exact',minzoom:DETAIL_MIN,layout:{visibility:'none'},paint:{'fill-extrusion-color':'#8a8f90','fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','base_height_m']],0]],'fill-extrusion-opacity':1,'fill-extrusion-vertical-gradient':true}},
  {id:'gta-exact-facade-detail',type:'fill-extrusion',source:'exact',minzoom:16.15,layout:{visibility:'none'},paint:{'fill-extrusion-pattern':facadePattern,'fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','base_height_m']],0]],'fill-extrusion-opacity':.999,'fill-extrusion-vertical-gradient':false}},
  {id:'gta-exact-roof',type:'fill-extrusion',source:'exactRoof',minzoom:DETAIL_MIN,layout:{visibility:'none'},paint:{'fill-extrusion-color':'#666c6d','fill-extrusion-base':['get','roof_base_m'],'fill-extrusion-height':['get','roof_top_m'],'fill-extrusion-opacity':.999,'fill-extrusion-vertical-gradient':true}},
...
function installBuildingMaterials(map){
 ...
 const facadeIds=facadeKinds.flatMap(k=>['bpfacade-solid-'+k+'-v0','bpfacade-'+k+'-v0']),roofIds=roofKinds.map(k=>'bproof-'+k),windowIds=['bp-window-night-warm','bp-window-night-cool','bp-window-night-mixed'];
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5356',mode:'rendered-vector-3d',aerialBase:false,roofCaps:true,directionalLight:true,facadeTextures:true,facadeArchetypes:facadeKinds.length,facadeLayoutVariants:128,possibleFacadeLayouts:facadeKinds.length*128,residentialLayoutVariants:10*128,officeLayoutVariants:9*128,medicalLayoutVariants:3*128,fireStationLayoutVariants:3*128,schoolLayoutVariants:3*128,barnArchetypes:3,roofArchetypes:roofKinds.length,deterministicPerBuilding:true,lazyVariantTextures:true,globalStyle:true,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5356',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function updateWorldLight(){
  ...
  try{
   ...
   const windowOpacity=alt>2?(map.getZoom()>=16.8?.12:0):clamp((-alt+2)/16,.08,.72);for(const id of ['gta-context-night-windows','gta-bp-night-windows','gta-exact-night-windows'])paint(map,id,'fill-extrusion-opacity',0);
   window.__BP_WORLD_LIGHT__={version:5356,sunAltitude:alt,sunAzimuth:sun.az,daylight:day,night:night>0.55,twilight,moonlight:night>0.45,updatedAt:Date.now()}
 ...
 function autoPitchForZoom(z){return z>=17.35?78:z>=16.4?64:z>=15.2?48:z>=13.6?26:0}
 function applyAutoCamera(delay=MOBILE?180:120){clearTimeout(autoCameraTimer);autoCameraTimer=setTimeout(()=>{if(moving||walkMode||autoCameraApplying)return;const target=autoPitchForZoom(map.getZoom());if(Math.abs(map.getPitch()-target)<2)return;autoCameraApplying=true;try{map.jumpTo({pitch:target})}catch(_){}setTimeout(()=>{autoCameraApplying=false},80);window.__BP_AUTO_CAMERA__={zoom:map.getZoom(),pitch:target,mode:target>=72?'STREET_FLY':target>=45?'3D_CITY':target>0?'TILT':'FLAT_OVERVIEW',updatedAt:Date.now()}},delay)}
 ...
 async function exact(){const z=map.getZoom();if(z<DETAIL_MIN||moving)return;const seq=++detailSeq,b=bbox(map,.13);lastExactFetchAt=performance.now();try{const d=await rpc('bridgepoint_building_viewport_v2300',{p_west:b.west,p_south:b.south,p_east:b.east,p_north:b.north,p_limit:MAX_EXACT},9000);if(seq!==detailSeq||moving)return;const p=await prepare(d?.features||[],MAX_EXACT);if(seq!==detailSeq||moving)return;if((p.buildings?.features?.length||0)>0){map.getSource('exact')?.setData(p.buildings);map.getSource('exactRoof')?.setData(p.roofs||EMPTY);exactCount=Number(p.count||0);syncBuildingShells()}window.__BP_EXACT_DETAIL_STATE__={status:exactCount>0?'ready':'empty',count:exactCount,visible:false,worldOverlay:false,inspectorAvailable:exactCount>0,zoom:z,updatedAt:Date.now()};setStatus(`BridgePoint World · persistent detail · ${exactCount.toLocaleString()} exact buildings retained · refinement visible`)}catch(e){if(seq===detailSeq){window.__BP_EXACT_DETAIL_STATE__={status:'error',count:exactCount,error:String(e?.message||e),updatedAt:Date.now()};setStatus(`BridgePoint World · persistent detail retained · background refinement retrying`)}}}
 ...
 function scheduleExact(delay=MOBILE?900:90){clearTimeout(detailTimer);detailTimer=setTimeout(()=>{if(!moving)exact()},delay)}
 ...
 function bridgePointBuildingCoverageReady(){
  ...
  try{
   ...
   if(!map.getLayer('gta-bp-buildings')||(map.getLayoutProperty('gta-bp-buildings','visibility')||'visible')==='none')return false;
   return (map.queryRenderedFeatures?.({layers:['gta-bp-buildings']})||[]).length>0
  }catch(_){return false}
 }
 function syncBuildingShells(){
  if(!layerState.buildings)return;
  const wantsBp=bridgePointDomesticCenter()&&map.getZoom()>=11.35;
  const now=performance.now(),quietReady=!MOBILE||(!moving&&now>=buildingShellQuietUntil),bpEligible=wantsBp&&quietReady,fineReady=!MOBILE||quietReady;
  // During a phone gesture and the short settle window, render exactly one building
  // extrusion pass. Roofs, shadows, windows, outlines and exact overlays return only
  // after the camera is quiet. This preserves buildings without paying 4–8 geometry
  // passes while a finger/camera is moving.
  vis(map,'gta-bp-buildings',bpEligible);
  for(const id of ['gta-bp-building-shadow','gta-bp-roofs','gta-bp-building-edge','gta-bp-night-windows'])vis(map,id,bpEligible&&fineReady);
  vis(map,'gta-bp-facade-detail',false);
  const useBp=bpEligible&&bridgePointBuildingCoverageReady();
  vis(map,'gta-context-buildings',!useBp);
  for(const id of ['gta-context-building-shadow','gta-context-roofs','gta-context-night-windows'])vis(map,id,!useBp&&fineReady);
  vis(map,'gta-context-facade-detail',false);
  const exactReady=exactCount>0&&map.getZoom()>=DETAIL_MIN&&fineReady;
  // Exact viewport geometry stays available to the inspector/X-ray pipeline but does
  // not stack a second world extrusion over the active shell.
  for(const id of ['gta-exact-building-shadow','gta-exact-building','gta-exact-roof','gta-exact-night-windows','gta-exact-facade-detail'])vis(map,id,false);
  vis(map,'gta-building-outline',false);
  clearTimeout(map.__bpBuildingShellProbe);
  if(wantsBp&&!useBp&&!moving||MOBILE&&!fineReady&&!moving){
   const wait=MOBILE?Math.max(140,buildingShellQuietUntil-performance.now()+80):120;
   map.__bpBuildingShellProbe=setTimeout(syncBuildingShells,wait)
  }
  window.__BP_BUILDING_SHELL_MODE__={authoritative:useBp?'BRIDGEPOINT':'GLOBAL_CONTEXT',requestedBridgePoint:wantsBp,bridgePointEligible:bpEligible,bridgePointCoverageReady:useBp,fallbackActive:wantsBp&&!useBp,zeroBuildingFramePrevented:true,mobileQuietHandoff:MOBILE,fineDetailDeferred:!fineReady,quietUntil:buildingShellQuietUntil,duplicateExactShell:false,updatedAt:Date.now()}
 }
 ...
 function movement(on){
  moving=on;
  if(on){
   if(MOBILE)buildingShellQuietUntil=performance.now()+120000;
   clearTimeout(detailTimer);clearTimeout(map.__bpLivingTimer);clearTimeout(lightTimer);clearTimeout(road3dTimer);clearTimeout(map.__bpBuildingShellProbe);
   if(MOBILE)syncBuildingShells();
   return
  }
  if(MOBILE)buildingShellQuietUntil=performance.now()+1400;
  terrain();syncBuildingShells();applyAutoCamera();rebuildRoad3D(MOBILE?3600:620);scheduleExact(MOBILE?2800:90);scheduleLivingWorld(MOBILE?3300:120);lights(MOBILE?2200:140)
 }
 function toggleLayer(name,on){if(name==='parcels'){layerState.parcels=!!on;for(const id of ['gta-parcel','gta-parcel-glow'])vis(map,id,on);if(on)startParcelFlow()}if(name==='buildings'){layerState.buildings=!!on;if(!on){for(const id of ['gta-context-building-shadow','gta-context-buildings','gta-context-facade-detail','gta-context-roofs','gta-context-night-windows','gta-bp-building-shadow','gta-bp-buildings','gta-bp-facade-detail','gta-bp-roofs','gta-bp-building-edge','gta-bp-night-windows','gta-exact-building-shadow','gta-exact-building','gta-exact-facade-detail','gta-exact-roof','gta-exact-night-windows','gta-building-outline'])vis(map,id,false)}else syncBuildingShells()}}
