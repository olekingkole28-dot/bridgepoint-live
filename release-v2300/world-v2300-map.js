...
function installBuildingMaterials(map){
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5343',mode:'rendered-vector-3d',aerialBase:false,roofCaps:true,directionalLight:true,facadeTextures:true,facadeArchetypes:facadeKinds.length,facadeLayoutVariants:12,possibleFacadeLayouts:facadeKinds.length*12,officeLayoutVariants:9*12,barnArchetypes:3,roofArchetypes:roofKinds.length,deterministicPerBuilding:true,lazyVariantTextures:true,globalStyle:true,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5343',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function syncBuildingShells(){
  ...
  // Two-phase shell switch: make the BridgePoint layer eligible first, but keep the
  // global shell underneath until a BridgePoint building is actually rendered in
  // this viewport. This makes a zero-building frame impossible during source gaps.
  for(const id of ['gta-bp-building-shadow','gta-bp-buildings','gta-bp-roofs','gta-bp-building-edge','gta-bp-night-windows'])vis(map,id,wantsBp);
  const useBp=wantsBp&&bridgePointBuildingCoverageReady();
  ...
  if(wantsBp&&!useBp&&!moving)map.__bpBuildingShellProbe=setTimeout(syncBuildingShells,MOBILE?240:120);
  window.__BP_BUILDING_SHELL_MODE__={authoritative:useBp?'BRIDGEPOINT':'GLOBAL_CONTEXT',requestedBridgePoint:wantsBp,bridgePointCoverageReady:useBp,fallbackActive:wantsBp&&!useBp,zeroBuildingFramePrevented:true,duplicateExactShell:false,updatedAt:Date.now()}
 }
 ...
 function movement(on){
  moving=on;
  if(on){
   clearTimeout(detailTimer);clearTimeout(map.__bpLivingTimer);clearTimeout(lightTimer);clearTimeout(road3dTimer);
   return
  }
