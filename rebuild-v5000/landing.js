...
function installLandingNationalWeather(world){const m=world?.map;if(!m)return;const empty={type:'FeatureCollection',features:[]},install=()=>{try{if(!m.getSource('bpLandingWeatherPoints'))m.addSource('bpLandingWeatherPoints',{type:'geojson',data:empty});if(!m.getSource('bpLandingWeatherShapes'))m.addSource('bpLandingWeatherShapes',{type:'geojson',data:empty});if(!m.getLayer('bp-landing-weather-fill'))m.addLayer({id:'bp-landing-weather-fill',type:'fill',source:'bpLandingWeatherShapes',minzoom:0,filter:['==',['geometry-type'],'Polygon'],paint:{'fill-color':landingWeatherColorExpr(),'fill-opacity':['case',['==',['get','observed'],true],.18,.10]}});if(!m.getLayer('bp-landing-weather-line'))m.addLayer({id:'bp-landing-weather-line',type:'line',source:'bpLandingWeatherShapes',minzoom:0,paint:{'line-color':landingWeatherColorExpr(),'line-width':['interpolate',['linear'],['zoom'],0,1.1,2.2,1.5,4,2,8,3,14,4.5],'line-opacity':['case',['==',['get','observed'],true],.94,.72]}});if(!m.getLayer('bp-landing-weather-glow'))m.addLayer({id:'bp-landing-weather-glow',type:'circle',source:'bpLandingWeatherPoints',minzoom:0,paint:{'circle-radius':['interpolate',['linear'],['zoom'],0,1.8,2.2,2.5,4.5,4,9,7,14,11],'circle-color':landingWeatherColorExpr(),'circle-opacity':['case',['==',['get','observed'],true],.18,.10],'circle-blur':.8}});if(!m.getLayer('bp-landing-weather-points'))m.addLayer({id:'bp-landing-weather-points',type:'circle',source:'bpLandingWeatherPoints',minzoom:0,paint:{'circle-radius':['interpolate',['linear'],['zoom'],0,.9,2.2,1.3,4.5,1.8,9,3.5,14,6],'circle-color':landingWeatherColorExpr(),'circle-opacity':['case',['==',['get','observed'],true],.9,.56],'circle-stroke-color':'#eefcff','circle-stroke-width':['interpolate',['linear'],['zoom'],0,.1,4.5,.25,12,1],'circle-stroke-opacity':.72}});if(!m.__bpLandingWeatherBound){m.__bpLandingWeatherBound=true;for(const id of ['bp-landing-weather-points','bp-landing-weather-fill','bp-landing-weather-line']){m.on('click',id,e=>landingWeatherPopup(m,e));m.on('mouseenter',id,()=>m.getCanvas().style.cursor='pointer');m.on('mouseleave',id,()=>{m.getCanvas().style.cursor=''})}}void loadLandingNationalWeather(world)}catch(e){console.warn('Landing national weather install',e)}};ensureLandingWeatherLegend();if(m.loaded()||m.isStyleLoaded?.())install();else m.once('load',install)}
async function loadLandingNationalWeather(world){try{const d=await rpc('bridgepoint_public_weather_bootstrap_v5004',{},5000),m=world?.map;if(!m)return;if(d?.fresh!==true){m.getSource('bpLandingWeatherPoints')?.setData({type:'FeatureCollection',features:[]});m.getSource('bpLandingWeatherShapes')?.setData({type:'FeatureCollection',features:[]});if($('landingWeatherCount'))$('landingWeatherCount').textContent='STALE';if($('landingWeatherTime'))$('landingWeatherTime').textContent='Public weather cache is refreshing.';return}const c=landingWeatherCollections(d.items||[]);m.getSource('bpLandingWeatherPoints')?.setData(c.points);m.getSource('bpLandingWeatherShapes')?.setData(c.shapes);if($('landingWeatherCount'))$('landingWeatherCount').textContent=(d.items?.length||0)+' EVENTS';if($('landingWeatherTime'))$('landingWeatherTime').textContent='Fresh '+new Date(d.generated_at).toLocaleTimeString()+' · national alerts + source weather context';window.__BP_LANDING_LIVE_WEATHER__={fresh:true,count:d.items?.length||0,shapeCount:c.shapes.features.length,pointCount:c.points.features.length,updatedAt:Date.now()}}catch(e){console.warn('Landing national weather',e)}}
function startLandingNationalWeather(world){installLandingNationalWeather(world);clearInterval(landingWeatherTimer);landingWeatherTimer=setInterval(()=>loadLandingNationalWeather(world),60000)}
...
// BP_V5420_DEPLOY_SYNC_2: full-US start + animated radar + exact national boundary layer ordering.
