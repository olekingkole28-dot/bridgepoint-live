...
function installBuildingMaterials(map){
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5344',mode:'rendered-vector-3d',aerialBase:false,roofCaps:true,directionalLight:true,facadeTextures:true,facadeArchetypes:facadeKinds.length,facadeLayoutVariants:12,possibleFacadeLayouts:facadeKinds.length*12,officeLayoutVariants:9*12,barnArchetypes:3,roofArchetypes:roofKinds.length,deterministicPerBuilding:true,lazyVariantTextures:true,globalStyle:true,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5344',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function updateWorldLight(){
  ...
  try{
   ...
   window.__BP_WORLD_LIGHT__={version:5339,sunAltitude:alt,sunAzimuth:sun.az,daylight:day,night:night>0.55,twilight,moonlight:night>0.45,updatedAt:Date.now()}
 ...
 function movement(on){
  ...
  terrain();syncBuildingShells();rebuildRoad3D(MOBILE?3600:620);scheduleExact(MOBILE?2800:90);scheduleLivingWorld(MOBILE?3300:120);lights(MOBILE?2200:140)
 }
