...
function style(){
 ...
 layers:[
  ...
  {id:'gta-bp-tree3d-trunk',type:'fill-extrusion',source:'bpLivingWorld3D',minzoom:15.0,filter:['==',['get','part'],'tree_trunk'],paint:{'fill-extrusion-color':'#5b412d','fill-extrusion-base':0,'fill-extrusion-height':['to-number',['get','top_m'],1],'fill-extrusion-opacity':.99,'fill-extrusion-vertical-gradient':true}},
  {id:'gta-bp-tree3d-canopy',type:'fill-extrusion',source:'bpLivingWorld3D',minzoom:15.0,filter:['==',['get','part'],'tree_canopy'],paint:{'fill-extrusion-color':['match',['get','leaf_cycle'],'evergreen','#285837','deciduous','#397344','#32673d'],'fill-extrusion-base':['to-number',['get','base_m'],0],'fill-extrusion-height':['to-number',['get','top_m'],1],'fill-extrusion-opacity':.96,'fill-extrusion-vertical-gradient':true}},
  ...
  {id:'gta-bp-tree-trunks',type:'circle',source:'bpLivingWorld',minzoom:14.6,maxzoom:15.05,filter:['==',['get','layer_key'],'TREE_OBJECTS_3D'],paint:{'circle-radius':['interpolate',['linear'],['zoom'],16,1,20,2.4],'circle-color':'#614a33','circle-opacity':.95}},
  {id:'gta-bp-tree-crowns',type:'circle',source:'bpLivingWorld',minzoom:14.6,maxzoom:15.05,filter:['==',['get','layer_key'],'TREE_OBJECTS_3D'],paint:{'circle-radius':['interpolate',['linear'],['zoom'],16,3,18,6,20,10],'circle-color':['interpolate',['linear'],['coalesce',['to-number',['get','height_m']],8],2,'#668c4d',8,'#47743f',20,'#315f37',35,'#294f31'],'circle-stroke-color':'#24472c','circle-stroke-width':1,'circle-opacity':.9,'circle-blur':.05}},
...
export function initWorld(options={}){
 ...
 function livingPhysical(data){
  ...
  for(const ft of data?.features||[]){if(used>=cap)break;const p=ft.properties||{},g=ft.geometry;if(!g)continue;
   ...
   if(p.layer_key==='TREE_OBJECTS_3D'||kind==='tree'){
    ...
    out.push({type:'Feature',geometry:{type:'Polygon',coordinates:circlePoly(coord,trunk,8)},properties:{part:'tree_trunk',top_m:base,height_truth:heightTruth,position_truth:positionTruth,truth:positionTruth}},{type:'Feature',geometry:{type:'Polygon',coordinates:circlePoly(coord,crown,12)},properties:{part:'tree_canopy',base_m:base,top_m:h,height_truth:heightTruth,position_truth:positionTruth,leaf_cycle:String(p.leaf_cycle||p.properties?.tags?.leaf_cycle||p.subtype||'').toLowerCase(),species:String(p.species||p.properties?.tags?.species||''),truth:heightTruth}});used++
 ...
 async function livingWorld(){
  if(map.getZoom()<14.2||moving)return;
  if(worldMapSurfaceInactive())return;
  if(worldClientPressureBlocked()){scheduleLivingWorld(MOBILE?700:180);return}
  const z=map.getZoom(),seq=++livingSeq,b=bbox(map,MOBILE?.035:.06),baseLimit=MOBILE?(z>=16.5?520:360):(TIER==='HIGH'?(z>=16.5?1450:1000):(z>=16.5?1050:760)),treeLimit=MOBILE?(z>=17?420:z>=16?300:180):(TIER==='HIGH'?(z>=17?1150:z>=16?820:420):(z>=17?800:z>=16?560:320));lastLivingFetchAt=performance.now();
  try{
   const settled=await Promise.allSettled([
    edge('bridgepoint-living-world-viewport-v5319',{west:b.west,south:b.south,east:b.east,north:b.north,limit:baseLimit},8500),
    edge('bridgepoint-global-tree-viewport-v1101',{west:b.west,south:b.south,east:b.east,north:b.north,limit:treeLimit},12000)
   ]);
   ...
   const data={type:'FeatureCollection',features};if(features.length){
    ...
    const physical=livingPhysical(physicalData);
    map.getSource('bpLivingWorld3D')?.setData(physical);
    const treeOn=(trees.features||[]).length>0&&z>=15;
    vis(map,'gta-bp-tree3d-trunk',treeOn);vis(map,'gta-bp-tree3d-canopy',treeOn);
    vis(map,'gta-bp-tree-trunks',(trees.features||[]).length>0&&z>=14.6&&z<15.05);
    vis(map,'gta-bp-tree-crowns',(trees.features||[]).length>0&&z>=14.6&&z<15.05);
    livingCount=features.length;
    window.__BP_TREE_RENDER_STATE__={version:1111,requested:(trees.features||[]).length,physicalFeatures:(physical.features||[]).length,visible3D:treeOn,trunkLayer:(map.getLayoutProperty('gta-bp-tree3d-trunk','visibility')||'visible'),canopyLayer:(map.getLayoutProperty('gta-bp-tree3d-canopy','visibility')||'visible'),zoom:z,updatedAt:Date.now()}
   }
 ...
 function schedulePostMoveSettle(){
  ...
  postMoveSettleTimer=setTimeout(()=>{
   ...
   if(map.getZoom()>=14.2)scheduleLivingWorld(MOBILE?900:120);
 ...
 const finalizeWorldCore=()=>{
  ...
  try{
   ...
   worldCoreReady=true;setTimeout(scheduleExact,MOBILE?350:220);if(map.getZoom()>=14.2)setTimeout(()=>scheduleLivingWorld(0),MOBILE?700:180);setStatus('BridgePoint World v5625 · owner-style fast tech map · white roofs · labels · 3D trees');return true
