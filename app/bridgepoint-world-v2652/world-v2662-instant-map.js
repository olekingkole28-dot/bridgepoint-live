...
function purgeLegacy(viewer){for(const name of LEGACY_LABEL_SOURCES){const ds=viewer.dataSources.getByName(name)[0];if(ds)viewer.dataSources.remove(ds,true)}}
...
async function buildCountries(viewer){
  ...
  for(const f of geo.features||[]){
    ...
    for(const rr of rings(f.geometry)){
      ...
      const e=ds.entities.add({polyline:{positions:Cesium.Cartesian3.fromDegreesArrayHeights(d),width:isUs?2.2:1.4,material:Cesium.Color.fromCssColorString(isUs?'#5cff9d':'#ff3d55').withAlpha(isUs?0.98:0.92),arcType:Cesium.ArcType.GEODESIC,distanceDisplayCondition:new Cesium.DistanceDisplayCondition(300000,26000000)}});if(!isUs)e.bridgepointCountry={code,name,locked:true};
    }
    if(isUs)continue;
    const c=center(ring),b=bbox(ring);if(!c)continue;
...
function saveWeatherCache(viewer){
  ...
  if(!rows.length)return;try{localStorage.setItem(WEATHER_CACHE_KEY,JSON.stringify({saved_at:Date.now(),rows}))}catch(_){}
}
function wireWeatherCache(viewer,world){
  let cached=restoreWeatherCache(viewer);const src=viewer.dataSources.getByName('bp-weather')[0];if(!src)return;let timer=0;
  ...
  src.entities.collectionChanged.addEventListener(changed);changed();world.loadWeather?.(true);world.refreshRadar?.();
}

function fastPrefetch(viewer,world){
  viewer.camera.percentageChanged=.004;let timer=0;
...
async function boot(){
  const world=await waitWorld(),viewer=world.viewer;forceUs(viewer);guardLegacy(viewer);wireWeatherCache(viewer,world);fastPrefetch(viewer,world);
