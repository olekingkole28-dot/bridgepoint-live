...
import{initAdminLabels}from'./world-v2300-admin-labels.js';
import{initStreetLabels}from'./world-v2300-street-labels.js';
import{initWaterLabels}from'./world-v2300-water-labels.js';
import{initStructureLabels}from'./world-v2300-structure-labels.js';
import{initOpportunitySync}from'./world-v2300-opportunity-sync.js';
import{initZoomContinuity}from'./world-v2300-zoom-continuity.js';
import{initStreetscapeDetail}from'./world-v2300-streetscape-detail.js';
import{initPaletteLock}from'./world-v2300-palette-lock.js';
import{initInspector}from'./world-v2300-inspector.js';

let booting=false,ready=false,world=null,details=null,weather=null,presentWeather=null,weatherKey=null,weatherPersistence=null,flythrough=null,buildingFidelity=null,roadStyle=null,boundaryContinuity=null,lockedCountries=null,landmarks=null,rail3d=null,adminLabels=null,streetLabels=null,waterLabels=null,structureLabels=null,opportunitySync=null,zoomContinuity=null,streetscapeDetail=null,paletteLock=null,inspector=null,extensionsBound=false;
...
function bindExtensions(){
  ...
  const attach=()=>{
    ...
    try{opportunitySync=initOpportunitySync(world.map)}catch(e){console.warn('V2300 opportunity sync init',e)}
    ...
    try{adminLabels=initAdminLabels(world.map)}catch(e){console.warn('V2300 admin labels init',e)}
    try{streetLabels=initStreetLabels(world.map)}catch(e){console.warn('V2300 street labels init',e)}
    try{waterLabels=initWaterLabels(world.map)}catch(e){console.warn('V2300 water labels init',e)}
    try{structureLabels=initStructureLabels(world.map)}catch(e){console.warn('V2300 structure labels init',e)}
    try{buildingFidelity=initBuildingFidelity(world.map)}catch(e){console.warn('V2300 building fidelity init',e)}
    try{zoomContinuity=initZoomContinuity(world.map)}catch(e){console.warn('V2300 zoom continuity init',e)}
    try{streetscapeDetail=initStreetscapeDetail(world.map)}catch(e){console.warn('V2300 streetscape detail init',e)}
    ...
    // Palette is intentionally the final additive style owner so later runtime modules cannot
    // quietly revert water, vegetation, pavement, or lane-language colors.
    try{paletteLock=initPaletteLock(world.map)}catch(e){console.warn('V2300 palette lock init',e)}
    expose();
  };
