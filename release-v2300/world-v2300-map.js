...
export function initWorld(options={}){
 ...
 function sky(){try{map.setSky({'sky-color':'#0b202d','horizon-color':'#385763','fog-color':'#627882','sky-horizon-blend':.18,'horizon-fog-blend':.055,'fog-ground-blend':.025,'atmosphere-blend':['interpolate',['linear'],['zoom'],0,.56,5,.31,9,.075,12,0]});map.setLight({anchor:'map',color:'#dedfd8',intensity:LOW?.5:TIER==='HIGH'?.62:.56,position:[1.15,205,48]})}catch(_){}}
 ...
 async function livingWorld(){
  if(map.getZoom()<14.2)return;const seq=++livingSeq,b=bbox(map,.08);
  try{
   ...
   const data=d?.type==='FeatureCollection'?d:EMPTY;map.getSource('bpLivingWorld')?.setData(data);livingCount=Number(data?.features?.length||0);
 ...
 function scheduleLivingWorld(){clearTimeout(map.__bpLivingTimer);if(map.getZoom()<14.2)return;map.__bpLivingTimer=setTimeout(()=>{if(!moving)livingWorld()},MOBILE?420:180)}
 ...
 function movement(on){
  moving=on;
  if(on){
   detailSeq++;livingSeq++;clearTimeout(detailTimer);clearTimeout(lightTimer);clearTimeout(map.__bpLivingTimer);return
  }
  terrain();scheduleExact();scheduleLivingWorld();lights()
 }
