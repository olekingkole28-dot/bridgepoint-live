...
const cap=TIER==='LOW'?220:TIER==='HIGH'?900:500;
const RAIL3D_MIN=TIER==='LOW'?14.9:TIER==='HIGH'?13.9:14.35;
...
export function initRail3D(map){
 ...
 function install(){
  ...
  add(map,{id:'gta-rail3d-bed',type:'fill-extrusion',source:'bp-rail3d-v2300',minzoom:RAIL3D_MIN,filter:['==',['get','part'],'bed'],paint:{'fill-extrusion-color':'#2e2d29','fill-extrusion-base':.015,'fill-extrusion-height':.095,'fill-extrusion-opacity':.98,'fill-extrusion-vertical-gradient':false}},'gta-context-buildings');
  add(map,{id:'gta-rail3d-ties',type:'fill-extrusion',source:'bp-rail3d-v2300',minzoom:RAIL3D_MIN+.22,filter:['==',['get','part'],'tie'],paint:{'fill-extrusion-color':'#8a643d','fill-extrusion-base':.09,'fill-extrusion-height':.155,'fill-extrusion-opacity':.99,'fill-extrusion-vertical-gradient':false}},'gta-context-buildings');
  add(map,{id:'gta-rail3d-steel',type:'fill-extrusion',source:'bp-rail3d-v2300',minzoom:RAIL3D_MIN,filter:['==',['get','part'],'steel'],paint:{'fill-extrusion-color':'#d9e2e4','fill-extrusion-base':.15,'fill-extrusion-height':.255,'fill-extrusion-opacity':1,'fill-extrusion-vertical-gradient':true}},'gta-context-buildings');
  installed=true;
 }
 function rebuild(){
  clearTimeout(timer);timer=setTimeout(()=>{
    if(map.getZoom()<RAIL3D_MIN){segments=0;polygons=0;try{map.getSource('bp-rail3d-v2300')?.setData(EMPTY)}catch(_){}return}
