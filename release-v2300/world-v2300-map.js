...
const BUILDINGS=`${EDGE}bridgepoint-public-building-tile-v5019?z={z}&x={x}&y={y}&limit=7000`;const CITY_STRUCTURES=`${EDGE}bridgepoint-public-city-structures-v5375?z={z}&x={x}&y={y}&limit=${MOBILE?500:650}&v=5376`;const CITY_ROOFS=`${EDGE}bridgepoint-public-city-roofs-v5376?z={z}&x={x}&y={y}&limit=${MOBILE?500:650}&v=5376`;
...
function style(){
 ...
 layers:[
  ...
  {id:'gta-context-roofs',type:'fill-extrusion',source:'ofm','source-layer':'building',minzoom:GLOBAL_ROOF_MIN,layout:{visibility:'none'},paint:{'fill-extrusion-color':'#6f6c65','fill-extrusion-base':['max',4,bHeight],'fill-extrusion-height':['+',['max',4,bHeight],['max',1.15,bRoofHeight]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],GLOBAL_ROOF_MIN,.86,14.8,.93,16,.98,18,1],'fill-extrusion-vertical-gradient':false}},
...
export function initWorld(options={}){
 ...
 map.on('movestart',()=>movement(true));map.on('moveend',()=>movement(false));map.on('zoomend',()=>{terrain();syncBuildingShells()});let bpCitySyncTimer=0;map.on('sourcedata',e=>{if(e?.sourceId!=='bpCityStructures'&&e?.sourceId!=='bpCityRoofs')return;clearTimeout(bpCitySyncTimer);bpCitySyncTimer=setTimeout(()=>{if(!moving)syncBuildingShells()},MOBILE?120:60)});
