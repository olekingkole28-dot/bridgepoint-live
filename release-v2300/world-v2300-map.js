...
window.__BP_WORLD_INTERACTION_V5556__={version:5556,exclusiveOverlayHits:true,nonBlockingBuildingOverlook:true,updatedAt:Date.now()};
...
export function initWorld(options={}){
 ...
 function chooseBuilding(e){if(window.__BP_MEASURE_ACTIVE__||performance.now()<Number(window.__BP_MAP_OVERLAY_CLICK_UNTIL__||0))return false;const visibleLayers=buildingHitLayers(true),knownLayers=buildingHitLayers(false);let f=Array.isArray(e?.features)?e.features.find(x=>knownLayers.includes(x?.layer?.id))||null:null;if(!f&&e?.point){try{f=(map.queryRenderedFeatures(e.point,{layers:visibleLayers})||[])[0]||null}catch(_){}}if(!f)return false;lastBuildingClickAt=performance.now();const p={...(f.properties||{})};if(!p.render_height_m)p.render_height_m=Number(p.height||p.render_height||8.5);const ll=e.lngLat||map.unproject(e.point);try{map.easeTo({center:[ll.lng,ll.lat],zoom:Math.max(map.getZoom(),16.6),pitch:64,bearing:-22,duration:MOBILE?180:240,essential:true})}catch(_){try{map.jumpTo({center:[ll.lng,ll.lat],zoom:Math.max(map.getZoom(),16.6),pitch:64,bearing:-22})}catch(__){}}const feature={type:'Feature',geometry:f.geometry,properties:p,id:f.id,layer:{id:f.layer?.id||''}},detail={lngLat:ll,feature};try{const panel=document.getElementById('buildingPanel');if(panel){panel.hidden=false;panel.removeAttribute('hidden');panel.style.setProperty('display','block','important');window.__BP_BUILDING_PANEL_OPEN__=true}}catch(_){}if(typeof window.__BP_V5000_SELECT_BUILDING__==='function'){try{window.__BP_V5000_SELECT_BUILDING__(detail)}catch(err){console.warn('v5000 building selector',err)}}else if(typeof buildingSelectHandler==='function'){try{buildingSelectHandler(detail)}catch(err){console.warn('building selection handler',err)}}window.dispatchEvent(new CustomEvent('bp2300:building-click',{detail}));window.__BP_BUILDING_SELECTION_V5547__={version:5547,panelImmediate:true,layer:f.layer?.id||'',updatedAt:Date.now()};return true}
