...
try {
  ...
  & $Py (Join-Path $Here "bridgepoint_agent.py") configure --data-root $DataRoot --primary-model $PrimaryModel --reviewer-model $ReviewerModel
} finally {
  $env:BRIDGEPOINT_ENROLLMENT_CODE = $null
  if ($bstr -ne [IntPtr]::Zero) { [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($bstr) }
}
$AgentPath = Join-Path $Here "bridgepoint_agent.py"
...
Write-Host "BridgePoint autonomous node v718 is installed."
