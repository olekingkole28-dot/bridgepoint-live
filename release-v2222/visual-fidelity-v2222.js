...
let map=null,state=null,bound=false,moving=false,restoreTimer=0,cityPitchDone=false,terrainOn=false,terrainSource=null;
...
function setSceneOwnership(){
 window.__bpVisualFidelityV2222={
  ...
  deviceTier:TIER,
  getState:()=>({version:VERSION,tier:TIER,zoom:map?.getZoom?.()||0,pitch:map?.getPitch?.()||0,terrainOn,terrainSource,sourceLoaded:{sat:sourceLoaded('sat'),dem:sourceLoaded('dem'),buildings:sourceLoaded('bp2220-ofm-direct')}})
 };
}
setSceneOwnership();
...
function terrain(){
 ...
 let current=null;try{current=map.getTerrain?.()||null}catch(_){}
 try{
  if(should){
   const already=terrainOn&&(terrainSource==='dem')&&(current===null||current?.source==='dem');
   if(!already){map.setTerrain?.({source:'dem',exaggeration:CFG.terrain});terrainOn=true;terrainSource='dem'}
  }else if(terrainOn||current){map.setTerrain?.(null);terrainOn=false;terrainSource=null}
 }catch(_){terrainOn=false;terrainSource=null}
}
...
function camera(){
 try{const p=map.getProjection?.()?.type;if(p!=='globe')map.setProjection?.({type:'globe'});map.setRenderWorldCopies?.(false);if(Number(map.getMaxPitch?.()||0)<CFG.pitch)map.setMaxPitch?.(CFG.pitch)}catch(_){}
...
function bind(){
 ...
 // A few startup passes let delayed legacy modules finish, then V2222 stays event-driven.
 for(const ms of [120,500,1400,3200])setTimeout(apply,ms);
