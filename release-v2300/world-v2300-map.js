import{VERSION,EDGE,EMPTY,MOBILE,LOW,TIER,rpc,tileTransform,bbox,fc,clamp}from'./world-v2300-config.js';
...
function style(){return{
 ...
 sources:{
  ...
  exact:{type:'geojson',data:EMPTY},exactRoof:{type:'geojson',data:EMPTY},selected:{type:'geojson',data:EMPTY},streetlights:{type:'geojson',data:EMPTY},bpOpportunities:{type:'geojson',data:EMPTY},bpSemanticLabels:{type:'geojson',data:EMPTY}
 },
 layers:[
  ...
  {id:'gta-water',type:'fill',source:'ofm','source-layer':'water',paint:{'fill-color':'#071f31','fill-opacity':.98}},
  {id:'gta-landcover',type:'fill',source:'ofm','source-layer':'landcover',minzoom:2,paint:{'fill-color':['match',['get','class'],'wood','#102b25','forest','#102b25','grass','#172d24','farmland','#29271d','ice','#dcecf1','#111b20'],'fill-opacity':['interpolate',['linear'],['zoom'],2,.58,8,.75,13,.84]}},
  {id:'gta-landuse',type:'fill',source:'ofm','source-layer':'landuse',minzoom:7.5,paint:{'fill-color':['match',['get','class'],'park','#13372b','grass','#183528','residential','#171f24','commercial','#202027','retail','#24212a','industrial','#1f272b','#1b2528'],'fill-opacity':.72}},
  {id:'gta-hillshade',type:'hillshade',source:'dem',minzoom:4,paint:{'hillshade-exaggeration':LOW?.34:TIER==='HIGH'?.62:.5,'hillshade-shadow-color':'#02070a','hillshade-highlight-color':'#b5d6d8','hillshade-accent-color':'#47626b'}},
  {id:'gta-road-major-glow',type:'line',source:'ofm','source-layer':'transportation',minzoom:6.5,filter:roadFilter(['motorway','trunk','primary','secondary']),layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#48e7ff','line-width':['interpolate',['exponential',1.35],['zoom'],7,1,11,2.8,15,9,19,20],'line-opacity':.18,'line-blur':3}},
  {id:'gta-road-major',type:'line',source:'ofm','source-layer':'transportation',minzoom:6.5,filter:roadFilter(['motorway','trunk','primary','secondary']),layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':['match',['get','class'],'motorway','#fff6c7','trunk','#d4fcff','primary','#a7f4ff','#79dce9'],'line-width':['interpolate',['exponential',1.35],['zoom'],7,.55,11,1.6,15,5.4,19,13],'line-opacity':.96}},
  {id:'gta-road-local-casing',type:'line',source:'ofm','source-layer':'transportation',minzoom:10.5,filter:roadFilter(['tertiary','minor','service','track']),layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#071017','line-width':['interpolate',['exponential',1.35],['zoom'],10.5,.8,14,2.6,17,7,20,15],'line-opacity':.94}},
  {id:'gta-road-local',type:'line',source:'ofm','source-layer':'transportation',minzoom:10.5,filter:roadFilter(['tertiary','minor','service','track']),layout:{'line-cap':'round','line-join':'round'},paint:{'line-color':'#c2f4f7','line-width':['interpolate',['exponential',1.35],['zoom'],10.5,.24,14,1,17,3.3,20,7.8],'line-opacity':['interpolate',['linear'],['zoom'],10.5,.4,14,.76,17,.92]}},
  ...
  {id:'gta-context-buildings',type:'fill-extrusion',source:'ofm','source-layer':'building',minzoom:11.2,paint:{'fill-extrusion-color':'#43545d','fill-extrusion-height':['max',4,['coalesce',['to-number',['get','render_height']],['to-number',['get','height']],['*',['coalesce',['to-number',['get','levels']],2],3],8]],'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','render_min_height']],['to-number',['get','min_height']],0]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],11.2,.66,13,.78,15,.88,18,.93],'fill-extrusion-vertical-gradient':true}},
  {id:'gta-bp-buildings',type:'fill-extrusion',source:'bpBuildings','source-layer':'buildings',minzoom:11.4,paint:{'fill-extrusion-color':['interpolate',['linear'],['coalesce',['to-number',['get','render_height_m']],8],0,'#55646c',18,'#7b8b92',55,'#9aa8ac',130,'#b9c4c7'],'fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-base':0,'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],11.4,.86,13,.92,15,.97,18,.99],'fill-extrusion-vertical-gradient':true}},
  {id:'gta-bp-building-edge',type:'line',source:'bpBuildings','source-layer':'buildings',minzoom:13.2,paint:{'line-color':'#90cbd4','line-width':['interpolate',['linear'],['zoom'],13.2,.3,18,.8,21,1.2],'line-opacity':.45}},
  ...
  {id:'gta-exact-building',type:'fill-extrusion',source:'exact',minzoom:DETAIL_MIN,paint:{'fill-extrusion-color':['match',['downcase',['coalesce',['get','facade_material'],'']],'brick','#9c6355','glass','#6b94a5','metal','#8a9ca3','concrete','#9b9d98','wood','#8f785f','#aeb9bb'],'fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','base_height_m']],0]],'fill-extrusion-opacity':.99,'fill-extrusion-vertical-gradient':true}},
  {id:'gta-exact-roof',type:'fill-extrusion',source:'exactRoof',minzoom:DETAIL_MIN,paint:{'fill-extrusion-color':['match',['downcase',['coalesce',['get','roof_material'],'']],'metal','#c6d6db','tile','#ad7b64','slate','#687985','shingle','#8d8580','concrete','#aab0ae','#bac7ca'],'fill-extrusion-base':['get','roof_base_m'],'fill-extrusion-height':['get','roof_top_m'],'fill-extrusion-opacity':.995,'fill-extrusion-vertical-gradient':false}},
  {id:'gta-building-outline',type:'line',source:'exact',minzoom:DETAIL_MIN,paint:{'line-color':'#dffcff','line-width':['interpolate',['linear'],['zoom'],14.5,.45,18,1.2,21,1.8],'line-opacity':.8}},
...
export function initWorld(){
 ...
 const map=new maplibregl.Map({container,style:style(),center:[-98.5,39.5],zoom:MOBILE?2.75:3.35,pitch:0,bearing:0,minZoom:2.2,maxZoom:21,projection:{type:'globe'},antialias:!MOBILE&&!LOW,fadeDuration:0,renderWorldCopies:false,transformRequest:tileTransform,maxTileCacheSize:MOBILE?36:(TIER==='LOW'?64:TIER==='HIGH'?180:110),crossSourceCollisions:!MOBILE,refreshExpiredTiles:!MOBILE,pixelRatio:MOBILE?1:undefined});
 ...
 let detailSeq=0,detailTimer=0,lightTimer=0,parcelPulseTimer=0,parcelPulsePhase=0,hoverFrame=0,exactCount=0,base='gta',moving=false,terrainOn=false,workerReq=0,lastTouchBuildingAt=0,touchPointer=null,touchNative=null,buildingSelectHandler=null,activeTouchPointers=new Set(),layerState={parcels:false,buildings:true};const worker=new Worker('./world-v2300-worker.js?v=2300',{type:'module'}),workerWait=new Map();
 ...
 function sky(){try{map.setSky({'sky-color':'#0d2a3b','horizon-color':'#42616e','fog-color':'#72858b','sky-horizon-blend':.2,'horizon-fog-blend':.05,'fog-ground-blend':.02,'atmosphere-blend':['interpolate',['linear'],['zoom'],0,.58,5,.34,9,.08,12,0]});map.setLight({anchor:'map',color:'#e7f6fb',intensity:.46,position:[1.2,205,38]})}catch(_){}}
 ...
 async function exact(){const z=map.getZoom(),seq=++detailSeq;if(z<DETAIL_MIN){map.getSource('exact')?.setData(EMPTY);map.getSource('exactRoof')?.setData(EMPTY);exactCount=0;return}const b=bbox(map,0);try{const d=await rpc('bridgepoint_building_viewport_v2300',{p_west:b.west,p_south:b.south,p_east:b.east,p_north:b.north,p_limit:MAX_EXACT},9000);if(seq!==detailSeq)return;const p=await prepare(d?.features||[],MAX_EXACT);if(seq!==detailSeq)return;map.getSource('exact')?.setData(p.buildings||EMPTY);map.getSource('exactRoof')?.setData(p.roofs||EMPTY);exactCount=Number(p.count||0);setStatus(`BridgePoint 3D world · ${exactCount.toLocaleString()} exact buildings in view · source-backed tiles + parcel truth`)}catch(e){if(seq===detailSeq)setStatus(exactCount?`BridgePoint 3D world · ${exactCount.toLocaleString()} exact buildings retained while detail refresh retries`:'BridgePoint 3D world · tile LOD active · exact detail will retry after movement') }}
 ...
 function movement(on){
  ...
  const extraFast=MOBILE?(map.__bpFastMotionLayers||[]):[];
  if(on){
    detailSeq++;
    ...
    if(MOBILE&&firstFast){
      ...
      for(const id of extraFast){map.__bpMotionRestoreVis[id]=map.__bpFastMotionDefaults?.[id]||'visible';vis(map,id,false)}
    }
 ...
 function chooseBuilding(e){if(window.__BP_MEASURE_ACTIVE__)return false;const layers=['gta-opportunity-buildings','gta-exact-building','gta-bp-buildings'].filter(id=>map.getLayer(id));let f=null;try{f=map.queryRenderedFeatures(e.point,{layers})[0]||null}catch(_){}if(!f)return false;const p={...(f.properties||{})};if(!p.render_height_m)p.render_height_m=Number(p.height||p.render_height||8.5);const ll=e.lngLat||map.unproject(e.point),feature={type:'Feature',geometry:f.geometry,properties:p,id:f.id,layer:{id:f.layer?.id||''}},z=Math.max(map.getZoom(),16.6);map.easeTo({center:ll,zoom:clamp(z,15,19),pitch:64,bearing:map.getBearing(),duration:LOW?260:520});const detail={lngLat:ll,feature};if(typeof window.__BP_V5000_SELECT_BUILDING__==='function'){try{window.__BP_V5000_SELECT_BUILDING__(detail)}catch(err){console.warn('v5000 building selector',err)}}else if(typeof buildingSelectHandler==='function'){try{buildingSelectHandler(detail)}catch(err){console.warn('building selection handler',err)}}window.dispatchEvent(new CustomEvent('bp2300:building-click',{detail}));return true}
 map.on('load',()=>{installBridgePointIcons(map);sky();terrain();bindUI();setBase('gta');if(MOBILE){const fastSources=new Set(['ofm','dem','bpBuildings','bpParcels','exact','exactRoof','streetlights','bpOpportunities','bpSemanticLabels']),motionIds=new Set(['gta-parcel','gta-parcel-glow','gta-poi-label','gta-opportunity-label','gta-exact-roof','gta-building-outline','gta-streetlights','gta-streetlights-glow','gta-context-buildings','gta-bp-buildings','gta-bp-building-edge','gta-exact-building','gta-opportunity-buildings','gta-poi-probe']),layers=map.getStyle()?.layers||[];map.__bpFastMotionLayers=layers.filter(l=>l?.source&&fastSources.has(l.source)&&!motionIds.has(l.id)).map(l=>l.id);map.__bpFastMotionDefaults=Object.fromEntries(layers.filter(l=>map.__bpFastMotionLayers.includes(l.id)).map(l=>[l.id,l.layout?.visibility||'visible']))}startParcelFlow();setTimeout(scheduleExact,MOBILE?1200:500);setTimeout(lights,MOBILE?1600:700);setStatus('BridgePoint national view · live context ready · motion-first rendering')});
 ...
 map.on('mousemove',e=>{if(hoverFrame)return;const point={x:e.point.x,y:e.point.y};hoverFrame=requestAnimationFrame(()=>{hoverFrame=0;try{const hit=map.queryRenderedFeatures(point,{layers:['gta-opportunity-buildings','gta-exact-building','gta-bp-buildings'].filter(id=>map.getLayer(id))}).length;map.getCanvas().style.cursor=hit?'pointer':''}catch(_){}})});
