...
function style(){
 ...
 layers:[
  ...
  {id:'gta-road3d-curb',type:'fill-extrusion',source:'bpRoad3D',minzoom:15.7,paint:{'fill-extrusion-color':'#e7e5de','fill-extrusion-base':['get','base_m'],'fill-extrusion-height':['get','curb_top_m'],'fill-extrusion-opacity':.94,'fill-extrusion-vertical-gradient':true}},
  {id:'gta-road3d-deck',type:'fill-extrusion',source:'bpRoad3D',minzoom:15.7,paint:{'fill-extrusion-color':['case',['==',['get','bridge'],true],'#0d0f11','#111315'],'fill-extrusion-base':['get','base_m'],'fill-extrusion-height':['get','deck_top_m'],'fill-extrusion-opacity':.99,'fill-extrusion-vertical-gradient':true}},
  ...
  {id:'gta-bp-tree3d-trunk',type:'fill-extrusion',source:'bpLivingWorld3D',minzoom:15.5,filter:['==',['get','part'],'tree_trunk'],paint:{'fill-extrusion-color':'#5b412d','fill-extrusion-base':0,'fill-extrusion-height':['get','top_m'],'fill-extrusion-opacity':.99,'fill-extrusion-vertical-gradient':true}},
  {id:'gta-bp-tree3d-canopy',type:'fill-extrusion',source:'bpLivingWorld3D',minzoom:15.5,filter:['==',['get','part'],'tree_canopy'],paint:{'fill-extrusion-color':['match',['get','leaf_cycle'],'evergreen','#285837','deciduous','#397344','#32673d'],'fill-extrusion-base':['get','base_m'],'fill-extrusion-height':['get','top_m'],'fill-extrusion-opacity':.96,'fill-extrusion-vertical-gradient':true}},
  {id:'gta-bp-streetlight3d-pole',type:'fill-extrusion',source:'bpLivingWorld3D',minzoom:16,filter:['==',['get','part'],'streetlight_pole'],paint:{'fill-extrusion-color':'#666d70','fill-extrusion-base':0,'fill-extrusion-height':['get','top_m'],'fill-extrusion-opacity':1,'fill-extrusion-vertical-gradient':true}},
  {id:'gta-bp-streetlight3d-lamp',type:'fill-extrusion',source:'bpLivingWorld3D',minzoom:16,filter:['==',['get','part'],'streetlight_lamp'],paint:{'fill-extrusion-color':'#ffe8a8','fill-extrusion-base':['get','base_m'],'fill-extrusion-height':['get','top_m'],'fill-extrusion-opacity':1,'fill-extrusion-vertical-gradient':false}},
...
function installBuildingMaterials(map){
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5333',mode:'rendered-vector-3d',aerialBase:false,roofCaps:true,directionalLight:true,facadeTextures:true,facadeArchetypes:facadeKinds.length,roofArchetypes:roofKinds.length,deterministicPerBuilding:true,globalStyle:true,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5333',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function updateWorldLight(){
  ...
  try{
   ...
   window.__BP_WORLD_LIGHT__={version:5332,sunAltitude:alt,sunAzimuth:sun.az,daylight:day,night:night>0.55,twilight,moonlight:night>0.45,updatedAt:Date.now()}
 ...
 function livingPhysical(data){
  const out=[],cap=MOBILE?180:(TIER==='HIGH'?650:360);let used=0;
 ...
 function rebuildRoad3D(){
  clearTimeout(road3dTimer);road3dTimer=setTimeout(()=>{if(moving)return;const z=map.getZoom(),src=map.getSource('bpRoad3D');if(!src)return;if(z<15.7){src.setData(EMPTY);return}
   const layers=['gta-road-major','gta-road-local','gta-road-supplement-bp','gta-bridge-deck'].filter(id=>map.getLayer(id));let fs=[];try{fs=map.queryRenderedFeatures({layers})||[]}catch(_){return}
   const out=[],seen=new Set(),cap=MOBILE?220:(TIER==='HIGH'?760:420);let used=0;
   outer:for(const ft of fs){const p=ft.properties||{},g=ft.geometry,lines=g?.type==='LineString'?[g.coordinates]:g?.type==='MultiLineString'?g.coordinates:[],bridge=String(p.brunnel||'').toLowerCase()==='bridge'||String(p.bridge||'').toLowerCase()==='yes',w=road3dWidth(p),base=bridge?clamp((Number(p.layer)||1)*2.5,1.5,14):.015;
    for(const line of lines)for(let i=0;i<line.length-1;i++){if(used>=cap)break outer;const a=line[i],b=line[i+1],key=[a[0].toFixed(5),a[1].toFixed(5),b[0].toFixed(5),b[1].toFixed(5),Math.round(w*10)].join(':');if(seen.has(key))continue;seen.add(key);const poly=ribbonPoly(a,b,w*.5+.32);if(!poly)continue;out.push({type:'Feature',geometry:{type:'Polygon',coordinates:poly},properties:{base_m:base,deck_top_m:base+.09,curb_top_m:base+.2,bridge,road_class:p.class||'',truth:'SOURCE_CENTERLINE_DERIVED_WIDTH'}});used++}
   }
   src.setData({type:'FeatureCollection',features:out});window.__BP_ROAD3D_STATE__={segments:used,retainedDuringMotion:true,derivedFromSourceCenterlines:true,updatedAt:Date.now()}
  },MOBILE?130:80)
 }
 ...
 function movement(on){
  ...
  terrain();syncBuildingShells();rebuildRoad3D();scheduleExact(MOBILE?45:25);scheduleLivingWorld(MOBILE?60:35);lights(MOBILE?70:40)
 }
