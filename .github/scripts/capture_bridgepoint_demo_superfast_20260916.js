...
async function card(page, kicker, title, sub, hold=800, progress=null) {
...
async function cityMontage(page) {
  await card(page,'NATIONAL 3D WORLD','15 major U.S. cities — one live spatial interface','Fast cross-country flyover. 3D building context stays active while the camera moves.',1300,5);
  for(let i=0;i<cities.length;i++){
    ...
    await card(page,'CITY '+String(i+1).padStart(2,'0')+' / '+cities.length,name,label+' • live 3D context',480,6+(i/cities.length)*18);
  }
}
...
async function searchAddress(page, q) {
  ...
  try {
    await results.first().waitFor({state:'visible',timeout:5500});
    const resultText = await results.first().innerText().catch(()=>q);
    await results.first().click();
    try { await page.locator('#buildingPanel').waitFor({state:'visible',timeout:3500}); } catch{}
    try {
...
async function tab(page, name, label, sub, progress) {
  ...
  try {
    ...
    await sleep(600);
    await card(page,'PROPERTY INTELLIGENCE',label,sub,520,progress);
    return true;
  } catch {
    return false;
  }
...
async function propertySequence(page,q,index,total){
  ...
  await card(page,'PROPERTY '+String(index+1).padStart(2,'0')+' / '+total,q,r.found?'Live BridgePoint address result selected • resolving exact building + parcel context':'Live lookup response: '+r.resultText.slice(0,150),850,base);
...
(async()=>{
  ...
  try{
    ...
    await card(page,'LIVE PRODUCT FILM','BridgePoint Intelligence','From a national 3D spatial index to individual property truth — captured directly from the production app.',1800,2);
    await cityMontage(page);
    await card(page,'DEEP DIVE','10 demo properties across the U.S.','Every section below is the actual production interface. No fake property values are composited into the capture.',1300,25);
    ...
    await card(page,'BUILT FOR DECISIONS','Search → render → inspect → verify','Property geometry, structure, source lineage and live context in one continuous workflow.',1400,96);
    await cinematicMove(page,-98.5,39.5,3.35,0,0,900);
    await card(page,'BRIDGEPOINT INTELLIGENCE','One map. The backend underneath it.','Production walkthrough complete • clean master ready for your voice-over.',1800,100);
  } catch(e){
