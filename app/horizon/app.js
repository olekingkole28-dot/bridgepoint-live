...
async function openModal(tab){
  ...
  if(tab==='ACCOUNT'){
    ...
    c.innerHTML=`<div class="eyebrow">OWNER ONLY · LIVE HORIZON TELEMETRY</div><h1>Everything happening in Horizon</h1><div class="owner-metrics">${[['ACTIVE',out.active_players],['ACCOUNTS',out.accounts],['TOTAL KILLS',out.total_kills],['MONSTER KILLS',out.monster_kills],['DEATHS',out.deaths],['WINS',out.wins],['PLAY HOURS',Math.round(Number(out.play_seconds||0)/3600)],['XP',out.xp],['EXPLORED CELLS',out.explored_cells],['LOOT CLAIMED',out.loot_claimed],['DEATH DROPS',out.death_drops],['EVENTS',out.events]].map(x=>`<article><h3>${x[0]}</h3><b>${Number(x[1]||0).toLocaleString()}</b></article>`).join('')}</div><h2>Recent tracked events</h2><div class="leaderboard">${(out.recent_events||[]).slice(0,50).map(e=>`<div class="leader-row"><b>#${e.event_id}</b><span>${escapeHtml(e.event_type)}</span><span>${escapeHtml(e.mode||'')}</span><span>${escapeHtml(String(e.value??''))}</span><span>${new Date(e.created_at).toLocaleTimeString()}</span></div>`).join('')||'<p>No events yet.</p>'}</div>`;
  }else if(tab==='ARMORY'){
