...
function addLabel(map,base){
  ...
  try{map.addLayer(base);return true}catch(e){
    // MapLibre builds without roof-elevated symbols still get the exact same building
    // anchor/collision behavior; only the vertical roof lift is omitted.
...
export function initStructureLabels(map){
  ...
  function install(){
    ...
    addLabel(map,{id:'gta-structure-label-v2300',type:'symbol',source:'bp-structure-labels-v2300',minzoom:15,layout:{
      'text-field':['get','name'],'text-font':['Noto Sans Regular'],'text-size':['interpolate',['linear'],['zoom'],15,9,17,10.5,20,12.8],
    ...
    },paint:{'text-color':categoryColor(),'text-opacity':['interpolate',['linear'],['zoom'],15,.42,17,.82,19,.96],'text-halo-color':'#071017','text-halo-width':1.25,'text-halo-blur':.35}});
  ...
  function rebuild(){
    clearTimeout(timer);timer=setTimeout(()=>{
      if(map.getZoom()<14.7){matched=0;try{map.getSource('bp-structure-labels-v2300')?.setData(EMPTY)}catch(_){}return}
      ...
      const out=[],seen=new Set(),cap=MOBILE?180:420;
      for(const poi of pois){if(out.length>=cap)break;const pp=poi.properties||{};if(!wanted(pp)||!poi.geometry||poi.geometry.type!=='Point')continue;const name=text(pp['name:en'],pp.name);if(!name)continue;
        const p=map.project(poi.geometry.coordinates);let bs=[];try{bs=map.queryRenderedFeatures([[p.x-12,p.y-12],[p.x+12,p.y+12]],{layers:buildingLayers})||[]}catch(_){}const b=bs[0];if(!b?.geometry)continue;
  ...
  const api={version:2300,map,requirementKey:'v2300_structure_labels_v1',rebuild,get state(){return{installed,matchedStructures:matched,buildingFootprintAnchored:true,sourcePOIMatched:true,roofElevationApplied:elevated,roofElevationFallbackSafe:true,collisionSafe:true,mapAligned:true,secondaryCanvases:0}}};window.__bpStructureLabelsV2300=api;return api;
}
