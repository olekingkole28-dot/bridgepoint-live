...
function patchDialog(){const d=document.getElementById('bp974-map-dialog');if(!d)return;d.querySelectorAll('.intel-marker').forEach(polishMarker);const close=d.querySelector('[data-close]');if(close&&!close.dataset.bp984Close){const old=close.onclick;close.onclick=e=>{try{old?.call(close,e);}finally{setTimeout(()=>{if(typeof window.BridgePointNavigateFlutterTabV984==='function')window.BridgePointNavigateFlutterTabV984('home');},80);}};close.dataset.bp984Close='1';}}
