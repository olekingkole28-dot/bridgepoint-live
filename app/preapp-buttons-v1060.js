...
function replace(el){if(!el||el.dataset.bp1060Button==='1'||el.closest('button'))return;const intent=intentFor(el),b=document.createElement('button');b.type='button';b.className=el.className+' bp1060-preapp-button';b.dataset.bp1060Button='1';b.dataset.bp1060Intent=intent;b.innerHTML=el.innerHTML+`<span class="bp1060-open">${labelFor(intent)} ${arrow}</span>`;b.addEventListener('click',()=>route(intent));el.replaceWith(b);}
function upgrade(){const root=document.getElementById('bp1055-deeptech');if(!root)return false;root.querySelectorAll('.bp1055d-card,.bp1055d-lab').forEach(replace);return true;}
let tries=0;const t=setInterval(()=>{tries++;upgrade();if(tries>180)clearInterval(t)},500);setTimeout(upgrade,0);setTimeout(upgrade,900);
window.BridgePointPreappButtonsV1060={upgrade,route};
})();