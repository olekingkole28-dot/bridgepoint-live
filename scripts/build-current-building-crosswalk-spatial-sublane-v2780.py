...
spec=importlib.util.spec_from_file_location("bp_crosswalk_base_v2780",BASE_PATH)
base=importlib.util.module_from_spec(spec)
spec.loader.exec_module(base)

def fetch_sublane(state,lane,lanes,sublane,sublanes,out_path):
    schema=pa.schema([
    ...
    try:
        while True:
            page=base.broker("local_spatial_sublane_page",state,{
            ...
            if int(page.get("count",len(rows)))!=len(rows):
                raise RuntimeError("sublane page count mismatch")
            ...
            if rows:
                ...
                if nxt<=after:
                    raise RuntimeError("non-monotonic sublane cursor")
                after=nxt
    ...
    if parent_expected is None or parent_expected<=0 or total<=0:
        raise RuntimeError(f"invalid sublane result state={state} lane={lane} sublane={sublane}/{sublanes}")
...
def main():
    ...
    ap.add_argument("--out-dir",default="rescue-v2780")
    a=ap.parse_args()
    ...
    local_rows,parent_expected,bbox=fetch_sublane(
        a.state,a.lane,a.lanes,a.sublane,a.sublanes,local_path
    )
    run_doc=base.broker("source_runs_spatial",a.state,{
        "lane":a.lane,"lanes":a.lanes
    },timeout=240)
    source_runs=list(run_doc.get("runs",[]))
    if not source_runs:
        raise RuntimeError("no source runs for parent lane")
    if int(run_doc.get("expected_lane_rows",0))!=parent_expected:
