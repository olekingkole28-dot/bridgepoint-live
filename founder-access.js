...
function render(host){
  ...
  const horizon=product==='HORIZON';
  if(horizon){
    host.innerHTML=`<div class="bp-founder"><span class="bp-founder-badge">HORIZON EARLY ACCESS · NO CHARGE</span><h2>Get into BridgePoint Horizon early.</h2><p>Join the early-access list while the world, combat and matchmaking are being finished. No payment is connected and this form cannot charge you.</p><form data-founder-access-form data-product="${esc(product)}">
    <div class="bp-founder-grid"><div class="bp-founder-field"><label>Player name</label><input name="full_name" autocomplete="name" maxlength="120" required></div><div class="bp-founder-field"><label>Email</label><input name="email" type="email" autocomplete="email" maxlength="254" required></div></div>
    <div class="bp-founder-grid"><div class="bp-founder-field"><label>Platform</label><select name="role_title"><option>Android</option><option>iPhone / iPad</option><option>PC</option><option>Xbox / controller</option><option>Multiple</option></select></div><div class="bp-founder-field"><label>First mode you want</label><select name="use_case"><option>Team Deathmatch</option><option>Year One Survival</option><option>Raid / Extraction</option><option>Island Last Stand</option><option>I want to test everything</option></select></div></div>
    <input type="hidden" name="company_name" value=""><input type="hidden" name="vertical" value="OTHER"><input type="hidden" name="state_code" value=""><input type="hidden" name="territory" value=""><input type="hidden" name="team_size" value="1"><input type="hidden" name="urgency" value="NOW"><input type="hidden" name="budget_range" value="">
    <div class="bp-founder-hp"><input name="website" tabindex="-1" autocomplete="off"></div>
    <button type="submit">JOIN HORIZON EARLY ACCESS</button><div class="bp-founder-status"></div>
    <p class="bp-founder-fine">Early access is an interest request, not a purchase. Stripe and paid checkout remain disconnected.</p></form></div>`;
  }else host.innerHTML=`<div class="bp-founder"><span class="bp-founder-badge">FOUNDER ACCESS · NO CHARGE TODAY</span><h2>Put your territory in front of BridgePoint.</h2><p>Tell us what you need. High-urgency business requests are automatically prioritized. Billing is not connected and this form cannot charge you.</p><form data-founder-access-form data-product="${esc(product)}">
