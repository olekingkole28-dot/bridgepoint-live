...
function saveAuth(data){
  ...
  try{const session={...data,expires_at:data.expires_at||Math.floor(Date.now()/1000)+(data.expires_in||3600)};localStorage.setItem('sb-xdfsjztwgsbmabshzsjw-auth-token',JSON.stringify(session));localStorage.setItem('bp-homepage-auth-v1990',JSON.stringify(session))}catch(_){ }
}
...
startCosmos();bind();refreshPulse();loadMedia();setInterval(()=>{if(!document.hidden&&!document.querySelector('.modal.show'))refreshPulse()},30000);
})();
