...
window.__BP_WORLD_RENDER_VERSION__=5355;
...
function installBuildingMaterials(map){
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5355',mode:'rendered-vector-3d',aerialBase:false,roofCaps:true,directionalLight:true,facadeTextures:true,facadeArchetypes:facadeKinds.length,facadeLayoutVariants:128,possibleFacadeLayouts:facadeKinds.length*128,residentialLayoutVariants:10*128,officeLayoutVariants:9*128,medicalLayoutVariants:3*128,fireStationLayoutVariants:3*128,schoolLayoutVariants:3*128,barnArchetypes:3,roofArchetypes:roofKinds.length,deterministicPerBuilding:true,lazyVariantTextures:true,globalStyle:true,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5355',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function updateWorldLight(){
  ...
  try{
   ...
   window.__BP_WORLD_LIGHT__={version:5355,sunAltitude:alt,sunAzimuth:sun.az,daylight:day,night:night>0.55,twilight,moonlight:night>0.45,updatedAt:Date.now()}
 ...
 function syncBuildingShells(){
  ...
  vis(map,'gta-bp-facade-detail',MOBILE&&bpEligible&&fineReady&&map.getZoom()>=16.15);
  ...
  vis(map,'gta-context-facade-detail',MOBILE&&!useBp&&fineReady&&map.getZoom()>=16.15);
  ...
  vis(map,'gta-exact-facade-detail',MOBILE&&exactReady&&map.getZoom()>=16.15);
