...
const SITE='https://bridgepointintelligence.online/app/?capture=restart-v7';
const OUT='demo-restart-v7';
...
async function shot(page,file){
 await page.screenshot({path:file,type:'jpeg',quality:90,timeout:15000});
}
...
(async()=>{
 ...
 const add=async(page,label,dur=1.5)=>{
  ...
  await shot(page,file);manifest.push({file:path.resolve(file),dur,label});n++;
 };
 // intro
 {
  ...
  await sleep(1200);await add(page,'intro',3);await browser.close();
 }
 // cities: fresh browser each time, no cumulative WebGL/pipe buildup
 for(let i=0;i<cities.length;i++){
  ...
  await sleep(500);await add(page,'city '+c[0],1.8);await browser.close();
 }
 // properties: fresh browser per property
 for(let i=0;i<props.length;i++){
  ...
  for(let j=0;j<views.length;j++){
   ...
   await sleep(550);await title(page,'PROPERTY INTELLIGENCE',v[2],v[3],base+j+1);await add(page,v[2]+' '+q,1.45);
  }
  await browser.close();
 }
 // outro
 {
  const {browser,page}=await makePage();await jump(page,-72.6734,41.7658,16.6,62,-18);
