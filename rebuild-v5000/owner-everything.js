...
function renderAcquisition(){
 ...
 $('ownerGeo').innerHTML=table(['Geography','Visitors','Events','Last seen'],states.slice(0,80).map(x=>'<tr><td><b>'+esc([x.country_code,x.state_code||x.region_code,x.state_name].filter(Boolean).join(' · ')||x.key||'Unknown')+'</b></td><td>'+fmt(x.unique_visitors??x.visitors??0)+'</td><td>'+fmt(x.events??x.event_count??0)+'</td><td>'+esc(when(x.last_seen_at||x.last_event_at))+'</td></tr>'));
...
function renderBackend(){
 ...
 $('ownerStates').innerHTML=table(['Jurisdiction','Phase','Overall','Properties','Sources','Tasks Q/R/F','Updated'],states.map(x=>'<tr><td><b>'+esc(x.state_code||x.jurisdiction_key||'—')+'</b> '+esc(x.state_name||x.jurisdiction_name||'')+'</td><td>'+pill(x.phase||x.status)+'</td><td>'+esc(x.overall_pct!=null?Number(x.overall_pct).toFixed(1)+'%':'—')+'</td><td>'+fmt(x.properties||x.canonical_properties||0)+'</td><td>'+fmt(x.sources_accepted||0)+' / '+fmt(x.sources_discovered||0)+'</td><td>'+fmt(x.source_tasks_queued||0)+' / '+fmt(x.source_tasks_running||0)+' / '+fmt(x.source_tasks_failed||0)+'</td><td>'+esc(when(x.last_activity_at||x.updated_at))+'</td></tr>'));
 $('ownerSourceRegistry').innerHTML=table(['Source','Jurisdiction','Category','Health','Rows / records','Checked'],sources.slice(0,250).map(x=>'<tr><td><b>'+esc(x.source_name||x.source_key||x.name||'source')+'</b></td><td>'+esc(x.state_code||x.jurisdiction_key||'—')+'</td><td>'+esc(x.category||x.source_type||'—')+'</td><td>'+pill(x.health||x.status||'—')+'</td><td>'+fmt(x.rows||x.record_count||x.records||0)+'</td><td>'+esc(when(x.last_checked_at||x.updated_at))+'</td></tr>'));
