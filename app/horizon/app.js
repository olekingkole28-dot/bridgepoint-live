...
const state={player:null,config:null,catalog:null,yearOne:null,party:null,match:null,worldCell:null,maps:[],selectedMap:null,selectedMode:'TDM',channel:null,peers:new Map(),killcam:new KillCamBuffer(),pickup:null,installPrompt:null,queueing:false,yearOneRolloverChecked:false,session:null,account:null,character:null,stats:null,activePlayers:0,owner:false,conusOutline:null,social:null};
...
async function refreshAuthState(){
  ...
  renderAccountState();syncPlayAvailability();if(state.account)await refreshSocialInbox();return state.session;
}
...
async function refreshSocialInbox(){
  if(!state.account){state.social=null;const b=$('friendsBadge');if(b)b.hidden=true;return null}
  try{
    state.social=await rpc('bridgepoint_horizon_social_inbox_v4341',{p_player_id:ident.id,p_player_secret:ident.secret});
    const count=(state.social?.incoming_requests?.length||0)+(state.social?.lobby_invites?.length||0),b=$('friendsBadge');
    if(b){b.textContent=String(count);b.hidden=count<=0}
    return state.social;
  }catch{return null}
}
...
async function bootstrap(){
  ...
  markMode();
  renderParty();
  await refreshActivePlayers();await detectOwner();await refreshSocialInbox();setInterval(refreshActivePlayers,10000);setInterval(()=>refreshSocialInbox().catch(()=>{}),8000);
  ...
  window.BP_HORIZON_LOBBY_V4341={
    ok:true,build:4341,player_id:state.player?.player_id,mode:state.selectedMode,
  ...
  window.BP_HORIZON_LOBBY_V4340=window.BP_HORIZON_LOBBY_V4341;
  window.BP_HORIZON_LOBBY_V4330=window.BP_HORIZON_LOBBY_V4341;
  window.BP_HORIZON_LOBBY_V4320=window.BP_HORIZON_LOBBY_V4341;
  connectPartySignal();
  setInterval(refreshParty,1800);
  const initialTab=(qs.get('tab')||'').trim().toUpperCase();
  if(initialTab&&['BATTLE_PASS','LOCKER','LOADOUTS','ARMORY','PARTY','FRIENDS','STORE','ARENA','WATCH','STATS','RULES','ABOUT','OWNER','ACCOUNT','CUSTOMIZE'].includes(initialTab))setTimeout(()=>openModal(initialTab),250);
}
...
async function openModal(tab){
  ...
  if(tab==='ACCOUNT'){
    ...
    c.innerHTML=`<div class="eyebrow">OFFICIAL HORIZON RULES · V4341</div><h1>How Horizon works</h1><div class="rules-grid">
      ...
      <article><h3>TEAM DEATHMATCH</h3><ul><li>100-player target, 50-vs-50 population, 150 health per player and no shield, with solo/duo/trio/squad parties and moderate bot backfill.</li><li>50 compact dense real-location maps. Players vote between two maps or Random, then enter a separate 10-second in-world freeze where the map is loaded and camera-look works but movement/combat are locked.</li><li>A staged wildfire circle closes over 30 minutes. Outside the safe circle costs exactly 25 health every second; bots also route toward safety.</li><li>Five starting class presets include SMG, assault-rifle, LMG, sniper and custom builds. Firearms use live damage/magazine/ammo stats and functional optic/barrel/muzzle/underbarrel/magazine/stock/grip attachments.</li><li>Each class has two tactical slots and one lethal slot. Death drops remain collectible through the same three-second dwell pickup, with five quick slots plus five backpack slots and capped reserve ammo.</li><li>After death, gameplay returns to the lobby rather than silently respawning the local player.</li></ul></article>
      ...
      <article><h3>ACCOUNTS, STORE & PASS</h3><ul><li>A free authenticated account is required to play. Handles are unique and case-insensitive.</li><li>Banners, profile icons and characters come from starter, weekly store, monthly battle-pass and prestige catalogs.</li><li>Character creator supports male, female or unspecified presentation and stylized presets with hair, eyes, colors, clothing, shoes, backpacks, headwear and glasses.</li><li>Permanent Year One revival listings are $20 for one restored life and $50 for all three lives. Checkout and tax collection remain disabled until Stripe is deliberately connected later.</li><li>Party text chat is squad-only; headset voice uses browser/device microphone and WebRTC support where available.</li></ul></article>
    </div>`;
  }else if(tab==='ABOUT'){
    ...
    const [out,reportsOut]=await Promise.all([rpc('bridgepoint_horizon_owner_audit_v4341',{}).catch(()=>null),rpc('bridgepoint_horizon_owner_reports_v4341',{}).catch(()=>({reports:[]}))]);
    if(!out){c.innerHTML='<div class="eyebrow">OWNER</div><h1>Owner access required</h1>';return}
    const ps=out.players||[],pays=out.purchases||[],signals=out.anticheat||[],reports=reportsOut?.reports||[];
    c.innerHTML=`<div class="eyebrow">OWNER / CO-OWNER · HORIZON BACKEND</div><h1>Accounts, gameplay, purchases & anti-cheat</h1><p class="account-callout">Checkout is not connected yet, so purchase rows remain empty until Stripe is deliberately enabled. Both designated BridgePoint platform owners can see this page.</p><div class="owner-metrics"><article><h3>PLAYERS</h3><b>${ps.length}</b></article><article><h3>ACTIVE</h3><b>${state.activePlayers||0}</b></article><article><h3>PURCHASES</h3><b>${pays.length}</b></article><article><h3>UNREVIEWED FLAGS</h3><b>${signals.filter(x=>!x.reviewed).length}</b></article></div><h2>Player audit</h2><div class="leaderboard">${ps.map(p=>`<div class="leader-row"><b>LV ${p.level||1}</b><span>@${escapeHtml(p.handle||'Survivor')}<small style="display:block;color:#789087">${escapeHtml(p.email||'')}</small></span><span>${escapeHtml(p.current_mode||'OFFLINE')}</span><span>${Math.floor(Number(p.play_seconds||0)/60)}M</span><span>${Number(p.kills||0)} K · P${p.prestige||0} · ${Number(p.anticheat_unreviewed||0)} FLAGS</span></div>`).join('')||'<p>No linked Horizon accounts yet.</p>'}</div><h2>Payments</h2><div class="leaderboard">${pays.map(p=>`<div class="leader-row"><b>${escapeHtml(p.status)}</b><span>${escapeHtml(p.sku)}</span><span>${money(p.amount_cents,p.currency)}</span><span>TAX ${money(p.tax_cents,p.currency)}</span><span>${new Date(p.created_at).toLocaleString()}</span></div>`).join('')||'<p>No Horizon payments recorded. Stripe is still disconnected.</p>'}</div><h2>Anti-cheat signals</h2><div class="leaderboard">${signals.slice(0,100).map(s=>`<div class="leader-row"><b>S${s.severity}</b><span>${escapeHtml(s.signal_type)}</span><span>${escapeHtml(String(s.observed??''))}</span><span>MAX ${escapeHtml(String(s.expected_max??''))}</span><span>${new Date(s.created_at).toLocaleString()}</span></div>`).join('')||'<p>No anti-cheat signals.</p>'}</div><h2>Player reports</h2><div class="leaderboard">${reports.slice(0,100).map(r=>`<div class="leader-row"><b>${escapeHtml(r.status)}</b><span>@${escapeHtml(r.reporter_handle)} → @${escapeHtml(r.reported_handle)}<small style="display:block;color:#789087">${escapeHtml(r.category)} · ${escapeHtml(r.details||'')}</small></span><span>${new Date(r.created_at).toLocaleString()}</span><span>${escapeHtml(r.reported_email||'')}</span><span><button data-report-review="${r.report_id}" data-report-status="RESOLVED">RESOLVE</button></span></div>`).join('')||'<p>No player reports.</p>'}</div>`;
  }else if(tab==='FRIENDS'){
    if(!state.account){c.innerHTML='<div class="eyebrow">FRIENDS</div><h1>Sign in to use Horizon social</h1>';return}
    const social=await refreshSocialInbox()||{friends:[],incoming_requests:[],outgoing_requests:[],lobby_invites:[],blocked:[]};
    c.innerHTML=`<div class="eyebrow">HORIZON SOCIAL</div><h1>Friends, lobby invites & safety</h1>
      <div class="social-search"><input id="friendSearch" maxlength="20" placeholder="Search unique handle"><button data-social-search>SEARCH</button></div>
      <div id="friendSearchResults" class="social-list"></div>
      <h2>Lobby invites</h2><div class="social-list">${(social.lobby_invites||[]).map(x=>`<article><b>@${escapeHtml(x.handle)}</b><span>${escapeHtml(x.mode||'TDM')} · expires ${new Date(x.expires_at).toLocaleTimeString()}</span><div><button data-invite-action="ACCEPT" data-id="${x.invite_id}">ACCEPT</button><button data-invite-action="DECLINE" data-id="${x.invite_id}">DECLINE</button></div></article>`).join('')||'<p>No pending lobby invites.</p>'}</div>
      <h2>Friend requests</h2><div class="social-list">${(social.incoming_requests||[]).map(x=>`<article><b>@${escapeHtml(x.handle)}</b><span>Wants to add you</span><div><button data-friend-action="ACCEPT" data-id="${x.request_id}">ACCEPT</button><button data-friend-action="DECLINE" data-id="${x.request_id}">DECLINE</button></div></article>`).join('')||'<p>No incoming friend requests.</p>'}</div>
      <h2>Friends</h2><div class="social-list">${(social.friends||[]).map(x=>`<article><b>@${escapeHtml(x.handle)} · LV ${x.level||1}</b><span>${x.online?('ONLINE · '+escapeHtml(x.mode||'LOBBY')):'OFFLINE'} · P${x.prestige||0}</span><div><button data-lobby-invite="${x.player_id}">INVITE</button><button data-report-player="${x.player_id}" data-handle="${escapeHtml(x.handle)}">REPORT</button><button data-unfriend="${x.player_id}">UNFRIEND</button><button data-block-player="${x.player_id}">BLOCK</button></div></article>`).join('')||'<p>No friends yet. Search a handle above.</p>'}</div>
      <h2>Blocked</h2><div class="social-list">${(social.blocked||[]).map(x=>`<article><b>@${escapeHtml(x.handle)}</b><div><button data-unblock-player="${x.player_id}">UNBLOCK</button></div></article>`).join('')||'<p>No blocked players.</p>'}</div>`;
  }else if(tab==='PARTY'){
    ...
    c.innerHTML=`<div class="eyebrow">WEEKLY HORIZON STORE</div><h1>Original 3D cosmetics, loadout unlocks & Year One revives</h1><p style="color:#95aaa0">The rotating catalog refreshes weekly. Second Chance ($20) and Full Revival ($50) remain permanently listed. Stripe/tax checkout is not connected yet, so nothing can charge a player today.</p><div class="store-grid">${items.map(s=>`<article class="store-card">${rewardArt(s)}<span class="eyebrow">${s.permanent?'PERMANENT · ':''}${escapeHtml(s.rarity)} · ${escapeHtml(s.category)}</span><h3>${escapeHtml(s.display_name)}</h3><div class="price">${money(s.price_cents,s.currency)}</div><button disabled>PREVIEW READY · CHECKOUT OWNER-LOCKED</button></article>`).join('')}</div>`;
  }else if(tab==='ARENA'){
    if(!state.maps.length)await refreshMapCatalog().catch(()=>{});
    c.innerHTML=`<div class="eyebrow">TDM MATCH VOTE POOL · 50 REAL LOCATIONS</div><h1>Two-map + Random vote every match.</h1><p style="color:#95aaa0">All 50 one-word arenas stay in the live match vote pool. Before a match, Horizon offers two real-location cells plus Random for a 10-second vote; the winning cell preloads before the in-world countdown.</p><div class="map-catalog">${state.maps.map((m,i)=>`<article class="map-card"><canvas class="map-mini" width="260" height="130" data-map-key="${escapeHtml(m.map_key)}"></canvas><div class="map-card-body"><span class="eyebrow">#${i+1} · ${escapeHtml(m.state_code)} · ${escapeHtml(m.biome||'DENSE WORLD CELL')}</span><h3>${escapeHtml(m.display_name)}</h3><small>${Number(m.span_km||0).toFixed(1)} km combat cell · MATCH VOTE POOL</small></div></article>`).join('')}</div>`;
...
$('modalContent').addEventListener('click',async e=>{
  ...
  if(e.target.closest('[data-social-search]')){
    const q=$('friendSearch')?.value.trim(),box=$('friendSearchResults');if(!q||q.length<2)return;
    const out=await rpc('bridgepoint_horizon_social_search_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_query:q}).catch(()=>({players:[]}));
    if(box)box.innerHTML=(out.players||[]).map(x=>`<article><b>@${escapeHtml(x.handle)} · LV ${x.level||1}</b><span>${x.online?'ONLINE':'OFFLINE'} · P${x.prestige||0}</span><div>${x.friend?'<small>FRIEND</small>':x.pending?'<small>REQUEST PENDING</small>':`<button data-friend-request="${x.player_id}">ADD FRIEND</button>`}<button data-report-player="${x.player_id}" data-handle="${escapeHtml(x.handle)}">REPORT</button><button data-block-player="${x.player_id}">BLOCK</button></div></article>`).join('')||'<p>No matching players.</p>';return;
  }
  const friendReq=e.target.closest('[data-friend-request]');if(friendReq){try{await rpc('bridgepoint_horizon_friend_request_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_target_player_id:friendReq.dataset.friendRequest});status('Friend request sent');openModal('FRIENDS')}catch(err){status(err.message)}return}
  const friendAction=e.target.closest('[data-friend-action]');if(friendAction){try{await rpc('bridgepoint_horizon_friend_request_action_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_request_id:friendAction.dataset.id,p_action:friendAction.dataset.friendAction});await refreshSocialInbox();openModal('FRIENDS')}catch(err){status(err.message)}return}
  const invite=e.target.closest('[data-lobby-invite]');if(invite){try{await rpc('bridgepoint_horizon_lobby_invite_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_target_player_id:invite.dataset.lobbyInvite});status('Lobby invite sent')}catch(err){status(err.message)}return}
  const inviteAction=e.target.closest('[data-invite-action]');if(inviteAction){try{await rpc('bridgepoint_horizon_lobby_invite_action_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_invite_id:inviteAction.dataset.id,p_action:inviteAction.dataset.inviteAction});await refreshParty();await refreshSocialInbox();openModal('FRIENDS')}catch(err){status(err.message)}return}
  const unfriend=e.target.closest('[data-unfriend]');if(unfriend){try{await rpc('bridgepoint_horizon_unfriend_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_target_player_id:unfriend.dataset.unfriend});await refreshSocialInbox();openModal('FRIENDS')}catch(err){status(err.message)}return}
  const block=e.target.closest('[data-block-player]');if(block){try{await rpc('bridgepoint_horizon_block_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_target_player_id:block.dataset.blockPlayer,p_block:true});await refreshSocialInbox();openModal('FRIENDS')}catch(err){status(err.message)}return}
  const unblock=e.target.closest('[data-unblock-player]');if(unblock){try{await rpc('bridgepoint_horizon_block_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_target_player_id:unblock.dataset.unblockPlayer,p_block:false});await refreshSocialInbox();openModal('FRIENDS')}catch(err){status(err.message)}return}
  const report=e.target.closest('[data-report-player]');if(report){const category=prompt('Report category: CHEATING, HARASSMENT, VOICE_ABUSE, GRIEFING, EXPLOIT, INAPPROPRIATE_NAME, OTHER','CHEATING');if(!category)return;const details=prompt('Optional details','')||'';try{await rpc('bridgepoint_horizon_report_player_v4341',{p_player_id:ident.id,p_player_secret:ident.secret,p_target_player_id:report.dataset.reportPlayer,p_category:category,p_details:details,p_match_id:state.match?.match_id||null});status('Private report submitted to BridgePoint owner review')}catch(err){status(err.message)}return}
  const review=e.target.closest('[data-report-review]');if(review){try{await rpc('bridgepoint_horizon_owner_report_action_v4341',{p_report_id:Number(review.dataset.reportReview),p_status:review.dataset.reportStatus||'RESOLVED',p_owner_notes:'Reviewed from Horizon owner panel'});openModal('OWNER')}catch(err){status(err.message)}return}
