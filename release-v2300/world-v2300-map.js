...
function style(){
 ...
 const facadeVariant=seed(127,12);
 ...
 layers:[
  ...
  {id:'gta-bp-buildings',type:'fill-extrusion',source:'bpBuildings','source-layer':'buildings',minzoom:11.4,paint:{'fill-extrusion-color':facadeColor,'fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','base_height_m']],0]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],11.4,.91,13,.965,15,.995,18,1],...(MOBILE?{}:{'fill-extrusion-pattern':facadePattern}),'fill-extrusion-vertical-gradient':true}},
  {id:'gta-bp-roofs',type:'fill-extrusion',source:'bpBuildings','source-layer':'buildings',minzoom:MOBILE?15.8:13.8,paint:{'fill-extrusion-color':roofColor,...(MOBILE?{}:{'fill-extrusion-pattern':roofPattern}),'fill-extrusion-base':['max',['coalesce',['to-number',['get','base_height_m']],0],['-',['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],['max',.18,['coalesce',['to-number',['get','roof_height_m']],.3]]]],'fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],13.8,.8,15,.94,18,.995],'fill-extrusion-vertical-gradient':true}},
  ...
  {id:'gta-exact-roof',type:'fill-extrusion',source:'exactRoof',minzoom:DETAIL_MIN,layout:{visibility:'none'},paint:{'fill-extrusion-color':roofColor,...(MOBILE?{}:{'fill-extrusion-pattern':roofPattern}),'fill-extrusion-base':['get','roof_base_m'],'fill-extrusion-height':['get','roof_top_m'],'fill-extrusion-opacity':.999,'fill-extrusion-vertical-gradient':true}},
...
function facadePatternImage(id){
 ...
 const solid=id.includes('bpfacade-solid-'),raw=id.replace('bpfacade-solid-','').replace('bpfacade-',''),vm=raw.match(/-v(\d+)$/),variant=vm?Number(vm[1])%12:0,kind=raw.replace(/-v\d+$/,'');
 const specs={
...
function installBuildingMaterials(map){
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5345',mode:'rendered-vector-3d',aerialBase:false,roofCaps:true,directionalLight:true,facadeTextures:true,facadeArchetypes:facadeKinds.length,facadeLayoutVariants:12,possibleFacadeLayouts:facadeKinds.length*12,officeLayoutVariants:9*12,barnArchetypes:3,roofArchetypes:roofKinds.length,deterministicPerBuilding:true,lazyVariantTextures:true,globalStyle:true,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5345',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function updateWorldLight(){
  ...
  try{
   ...
   window.__BP_WORLD_LIGHT__={version:5345,sunAltitude:alt,sunAzimuth:sun.az,daylight:day,night:night>0.55,twilight,moonlight:night>0.45,updatedAt:Date.now()}
 ...
 function terrain(){const z=map.getZoom(),should=z>=6.5&&z<15.25,ex=z>=14.5?.28:z>=13.2?.58:(LOW?.9:TIER==='HIGH'?1.08:1);try{if(should){map.setTerrain({source:'dem',exaggeration:ex});terrainOn=true}else if(terrainOn){map.setTerrain(null);terrainOn=false}window.__BP_TERRAIN_VISUAL_STATE__={zoom:z,enabled:should,exaggeration:should?ex:0,streetLevelClamped:z>=15.25,coastRoadCliffProtection:true,updatedAt:Date.now()}}catch(_){terrainOn=false}}
 ...
 async function exact(){const z=map.getZoom();if(z<DETAIL_MIN||moving)return;const seq=++detailSeq,b=bbox(map,.13);lastExactFetchAt=performance.now();try{const d=await rpc('bridgepoint_building_viewport_v2300',{p_west:b.west,p_south:b.south,p_east:b.east,p_north:b.north,p_limit:MAX_EXACT},9000);if(seq!==detailSeq||moving)return;const p=await prepare(d?.features||[],MAX_EXACT);if(seq!==detailSeq||moving)return;if((p.buildings?.features?.length||0)>0){map.getSource('exact')?.setData(p.buildings);map.getSource('exactRoof')?.setData(p.roofs||EMPTY);exactCount=Number(p.count||0);syncBuildingShells()}window.__BP_EXACT_DETAIL_STATE__={status:exactCount>0?'ready':'empty',count:exactCount,visible:exactCount>0&&z>=DETAIL_MIN,zoom:z,updatedAt:Date.now()};setStatus(`BridgePoint World · persistent detail · ${exactCount.toLocaleString()} exact buildings retained · refinement visible`)}catch(e){if(seq===detailSeq){window.__BP_EXACT_DETAIL_STATE__={status:'error',count:exactCount,error:String(e?.message||e),updatedAt:Date.now()};setStatus(`BridgePoint World · persistent detail retained · background refinement retrying`)}}}
 ...
 function syncBuildingShells(){
  ...
  const now=performance.now(),quietReady=!MOBILE||(!moving&&now>=buildingShellQuietUntil),bpEligible=wantsBp&&quietReady;
  // Mobile interaction rule: keep the lightweight global shell visible while the
  // camera moves and for a short quiet period afterward. BridgePoint's heavier
  // authoritative vector tiles are only activated after the camera is settled.
  for(const id of ['gta-bp-building-shadow','gta-bp-buildings','gta-bp-roofs','gta-bp-building-edge','gta-bp-night-windows'])vis(map,id,bpEligible);
  const useBp=bpEligible&&bridgePointBuildingCoverageReady();
  for(const id of ['gta-context-building-shadow','gta-context-buildings','gta-context-roofs','gta-context-night-windows'])vis(map,id,!useBp);
  const exactReady=exactCount>0&&map.getZoom()>=DETAIL_MIN;
  for(const id of ['gta-exact-building-shadow','gta-exact-building','gta-exact-roof','gta-exact-night-windows'])vis(map,id,exactReady);
  vis(map,'gta-building-outline',exactReady);
  clearTimeout(map.__bpBuildingShellProbe);
  if(wantsBp&&!useBp&&!moving){
 ...
 function movement(on){
  ...
  if(MOBILE)buildingShellQuietUntil=performance.now()+2200;
 ...
 function toggleLayer(name,on){if(name==='parcels'){layerState.parcels=!!on;for(const id of ['gta-parcel','gta-parcel-glow'])vis(map,id,on);if(on)startParcelFlow()}if(name==='buildings'){layerState.buildings=!!on;if(!on){for(const id of ['gta-context-building-shadow','gta-context-buildings','gta-context-roofs','gta-context-night-windows','gta-bp-building-shadow','gta-bp-buildings','gta-bp-roofs','gta-bp-building-edge','gta-bp-night-windows','gta-building-outline'])vis(map,id,false)}else syncBuildingShells()}}
