...
window.__BP_WORLD_RENDER_VERSION__=5548;
...
const PARCEL_MIN=MOBILE?8.35:(TIER==='LOW'?8.55:TIER==='HIGH'?7.85:8.15);
...
function style(){
 ...
 sources:{
  ...
  bpParcels:{type:'vector',tiles:[PARCELS],minzoom:7,maxzoom:22,attribution:'BridgePoint parcel provenance'},
  exact:{type:'geojson',data:EMPTY},exactRoof:{type:'geojson',data:EMPTY},selected:{type:'geojson',data:EMPTY},streetlights:{type:'geojson',data:EMPTY},bpOpportunities:{type:'geojson',data:EMPTY},bpSemanticLabels:{type:'geojson',data:EMPTY},bpLivingWorld:{type:'geojson',data:EMPTY,attribution:'BridgePoint source-backed living-world detail · public/open government and OSM sources'},bpLivingWorld3D:{type:'geojson',data:EMPTY,attribution:'BridgePoint source-backed positions · derived display geometry'},bpLandmark3D:{type:'geojson',data:EMPTY,attribution:'OpenStreetMap/OpenFreeMap landmark POIs · BridgePoint source-position-derived 3D landmark geometry · Statue of Liberty height: U.S. National Park Service'},bpBridge3D:{type:'geojson',data:EMPTY,attribution:'OpenStreetMap/OpenFreeMap bridge centerlines · BridgePoint derived 3D bridge display geometry'},bpRoad3D:{type:'geojson',data:EMPTY,attribution:'OpenStreetMap/OpenFreeMap centerlines · BridgePoint derived physical road deck'}
 },
 layers:[
  ...
  {id:'gta-oam',type:'raster',source:'oam',minzoom:14,maxzoom:22,layout:{visibility:'none'},paint:{'raster-opacity':['interpolate',['linear'],['zoom'],14,0,14.25,.72,15,.9,17,.96,20,.985],'raster-saturation':-.02,'raster-contrast':.06,'raster-fade-duration':0}},
  ...
  {id:'gta-city-buildings',type:'fill-extrusion',source:'bpCityStructures','source-layer':'buildings',minzoom:11,layout:{visibility:'none'},paint:{'fill-extrusion-color':facadeColor,'fill-extrusion-height':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','base_height_m']],0]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],11,.88,12.5,.95,15,.995],'fill-extrusion-vertical-gradient':true}},
  {id:'gta-city-building-roof-caps',type:'fill-extrusion',source:'bpCityStructures','source-layer':'buildings',minzoom:GLOBAL_ROOF_MIN,layout:{visibility:'none'},paint:{'fill-extrusion-color':roofColor,'fill-extrusion-base':['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],'fill-extrusion-height':['+',['max',3,['coalesce',['to-number',['get','render_height_m']],8.5]],['max',.18,['coalesce',['to-number',['get','roof_height_m']],.55]]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],11,.92,13,.98,16,1],'fill-extrusion-vertical-gradient':true}},
  {id:'gta-city-roofs',type:'fill-extrusion',source:'bpCityRoofs','source-layer':'roofs',minzoom:15.85,layout:{visibility:'none'},paint:{'fill-extrusion-color':roofColor,'fill-extrusion-base':['max',0,['coalesce',['to-number',['get','roof_base_m']],0]],'fill-extrusion-height':['max',.18,['coalesce',['to-number',['get','roof_top_m']],.3]],'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],15.85,.88,17,.98,19,1],'fill-extrusion-vertical-gradient':true}},
  {id:'gta-context-buildings',type:'fill-extrusion',source:'ofm','source-layer':'building',minzoom:11.2,paint:{'fill-extrusion-color':facadeColor,'fill-extrusion-height':['max',4,bHeight],'fill-extrusion-base':bBase,'fill-extrusion-opacity':['interpolate',['linear'],['zoom'],11.2,.72,13,.86,15,.96,18,.995],'fill-extrusion-vertical-gradient':true}},
  ...
  {id:'gta-exact-roof',type:'fill-extrusion',source:'exactRoof',minzoom:DETAIL_MIN,filter:['!=',['get','roof_truth'],'VISUAL_CAP'],layout:{visibility:'none'},paint:{'fill-extrusion-color':roofColor,'fill-extrusion-base':['get','roof_base_m'],'fill-extrusion-height':['get','roof_top_m'],'fill-extrusion-opacity':.999,'fill-extrusion-vertical-gradient':true}},
...
export function initWorld(options={}){
 ...
 function startParcelFlow(){clearInterval(parcelPulseTimer);parcelPulseTimer=setInterval(()=>{if(!layerState.parcels||map.getZoom()<PARCEL_MIN)return;parcelPulsePhase=(parcelPulsePhase+1)%16;const t=(Math.sin((parcelPulsePhase/16)*Math.PI*2)+1)/2;paint(map,'gta-parcel-glow','line-opacity',.34+.16*t);paint(map,'gta-parcel-glow','line-blur',2.5+1.6*t);paint(map,'gta-parcel','line-opacity',.96+.04*t)},420)}
 ...
 function syncParcelShells(){const z=map.getZoom(),on=!!layerState.parcels&&bridgePointDomesticCenter()&&z>=PARCEL_MIN;vis(map,'gta-parcel-glow',on);vis(map,'gta-parcel-halo',on);vis(map,'gta-parcel',on);return on}
 ...
 function toggleLayer(name,on){if(name==='parcels'){layerState.parcels=!!on;syncParcelShells();if(on)startParcelFlow()}if(name==='buildings'){layerState.buildings=!!on;if(!on){for(const id of ['gta-context-building-footprints','gta-context-building-shadow','gta-context-buildings','gta-context-facade-detail','gta-context-building-parts','gta-context-roofs','gta-context-night-windows','gta-bp-building-shadow','gta-bp-buildings','gta-bp-facade-detail','gta-bp-roofs','gta-bp-building-edge','gta-bp-night-windows','gta-exact-building-shadow','gta-exact-building','gta-exact-facade-detail','gta-exact-roof','gta-exact-night-windows','gta-building-outline'])vis(map,id,false)}else syncBuildingShells()}}
 ...
 function chooseBuilding(e){if(window.__BP_MEASURE_ACTIVE__)return false;const visibleLayers=buildingHitLayers(true),knownLayers=buildingHitLayers(false);let f=Array.isArray(e?.features)?e.features.find(x=>knownLayers.includes(x?.layer?.id))||null:null;if(!f&&e?.point){try{f=(map.queryRenderedFeatures(e.point,{layers:visibleLayers})||[])[0]||null}catch(_){}}if(!f)return false;lastBuildingClickAt=performance.now();const p={...(f.properties||{})};if(!p.render_height_m)p.render_height_m=Number(p.height||p.render_height||8.5);const ll=e.lngLat||map.unproject(e.point);try{map.stop()}catch(_){};map.jumpTo({center:[ll.lng,ll.lat],zoom:Math.max(map.getZoom(),16.6),pitch:64,bearing:-22});const feature={type:'Feature',geometry:f.geometry,properties:p,id:f.id,layer:{id:f.layer?.id||''}},detail={lngLat:ll,feature};try{const panel=document.getElementById('buildingPanel');if(panel){panel.hidden=false;panel.removeAttribute('hidden');panel.style.setProperty('display','block','important');window.__BP_BUILDING_PANEL_OPEN__=true}}catch(_){}if(typeof window.__BP_V5000_SELECT_BUILDING__==='function'){try{window.__BP_V5000_SELECT_BUILDING__(detail)}catch(err){console.warn('v5000 building selector',err)}}else if(typeof buildingSelectHandler==='function'){try{buildingSelectHandler(detail)}catch(err){console.warn('building selection handler',err)}}window.dispatchEvent(new CustomEvent('bp2300:building-click',{detail}));window.__BP_BUILDING_SELECTION_V5547__={version:5547,panelImmediate:true,layer:f.layer?.id||'',updatedAt:Date.now()};return true}
  function bindBuildingHitLayers(){for(const id of ['gta-exact-building','gta-city-buildings','gta-context-buildings']){if(!map.getLayer(id))continue;map.on('click',id,e=>chooseBuilding(e));map.on('mouseenter',id,()=>{map.getCanvas().style.cursor='pointer'});map.on('mouseleave',id,()=>{map.getCanvas().style.cursor=''})}}
 ...
 canvas.addEventListener('click',ev=>{if(performance.now()-lastBuildingClickAt<220)return;nativeBuildingTap(ev.clientX,ev.clientY,ev)},{passive:true,capture:true});
