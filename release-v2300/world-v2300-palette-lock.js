...
const ASPHALT='#15191c',LOCAL_ASPHALT='#1b1f22',GOLD='#f2c94c',ROAD_EDGE='#f0f2ed';
...
export function initPaletteLock(map){
 ...
 function apply(){
  ...
  try{
   // Dynamic water, glacier and vegetation palettes belong to the shared renderer.
 ...
 const api={version:2300,map,requirementKey:'v2300_material_palette_lock_v1',apply,get state(){return{installed,applyCount,blueWater:has(map,'gta-water'),greenVegetation:has(map,'gta-landcover'),blackMajorPavement:has(map,'gta-road-major')&&same(map.getPaintProperty('gta-road-major','line-color'),ASPHALT),blackLocalPavement:has(map,'gta-road-local')&&same(map.getPaintProperty('gta-road-local','line-color'),LOCAL_ASPHALT),yellowWhiteRoadLanguage:true,paletteOwner:'ROAD_LANGUAGE_ONLY_V5332',secondaryCanvases:0}}};
