...
function installBuildingMaterials(map){
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5342',mode:'rendered-vector-3d',aerialBase:false,roofCaps:true,directionalLight:true,facadeTextures:true,facadeArchetypes:facadeKinds.length,facadeLayoutVariants:12,possibleFacadeLayouts:facadeKinds.length*12,officeLayoutVariants:9*12,barnArchetypes:3,roofArchetypes:roofKinds.length,deterministicPerBuilding:true,lazyVariantTextures:true,globalStyle:true,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5342',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function bridgePointBuildingCoverageReady(){
  ...
  try{
   const src=map.getSource('bpBuildings');
   if(!src||!map.isSourceLoaded?.('bpBuildings'))return false;
 ...
 function syncBuildingShells(){
  ...
  // Never allow a blank-building viewport: the global OpenStreetMap/OpenFreeMap shell
  // remains the fallback until BridgePoint's authoritative building tiles are loaded
  // and have actual features for the current viewport.
  ...
  window.__BP_BUILDING_SHELL_MODE__={authoritative:useBp?'BRIDGEPOINT':'GLOBAL_CONTEXT',requestedBridgePoint:wantsBp,bridgePointCoverageReady:useBp,fallbackActive:wantsBp&&!useBp,duplicateExactShell:false,updatedAt:Date.now()}
 }
