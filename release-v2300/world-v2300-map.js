...
window.__BP_WORLD_RENDER_VERSION__=5364;
...
function installBuildingMaterials(map){
 ...
 window.__BP_BUILDING_MATERIALS__={version:'v5364',mode:'simple-grey-3d',simpleGreyGeometry:true,legacyPersistentGlobalShell:true,facadeTextures:false,roofOverlays:false,buildingParts:false,nightWindows:false,exactWorldOverlay:false,updatedAt:Date.now()}
}
...
function installWorldMaterials(map){
 ...
 window.__BP_WORLD_MATERIALS__={version:'v5364',biomes:true,glacierRelief:true,physicalRoads:true,blackYellowWhiteRoads:true,bridgeDecks:true,shorelineMotion:true,landmarks:true,updatedAt:Date.now()}
}
...
export function initWorld(options={}){
 ...
 function updateWorldLight(){
  ...
  try{
   ...
   window.__BP_WORLD_LIGHT__={version:5364,sunAltitude:alt,sunAzimuth:sun.az,daylight:day,night:night>0.55,twilight,moonlight:night>0.45,updatedAt:Date.now()}
 ...
 function syncBuildingShells(){
  if(!layerState.buildings){
   for(const id of ['gta-context-building-footprints','gta-context-buildings','gta-bp-buildings','gta-exact-building'])vis(map,id,false);
   return
  }
  const z=map.getZoom();
  // V5364 restores the pre-handoff building renderer used in the earlier map:
  // one persistent global polygon extrusion, always grounded, never replaced.
  vis(map,'gta-context-building-footprints',z<11.2);
  ...
  window.__BP_BUILDING_SHELL_MODE__={
   authoritative:'GLOBAL_CONTEXT_LEGACY',
   legacyPersistent:true,
   bridgePointHandoff:false,
   immediate:true,
   ...
   displaySource:'OPENFREEMAP_GLOBAL_BUILDING',
   updatedAt:Date.now()
  }
 }
